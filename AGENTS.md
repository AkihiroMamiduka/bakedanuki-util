...
手書きの Python コードでは、公開 API の使い方を IDE 上で確認できるようにします。
docstring とコード内コメントは日本語で書き、docstring の節見出しには Google style の
`Args:`、`Returns:`、`Raises:` などを使います。
補足事項は `Notes:` などの独自節を増やさず、短い本文にまとめます。

- 公開クラス・関数・メソッド・プロパティには、まず短い一文で役割を示します。
  一文で十分な場合は節を増やしません。
- 本文・各節・コメントで引数や API の識別子に言及するときは、コードと同じ綴りを
  バッククォートで囲んで記します。識別子を和訳せず、似た引数の違いや併用条件も
  対象の識別子を明記して説明します（例: `start_frame` と `to_start_frame`）。
- `Args:` には引数の制約、単位、既定値や `None` の意味を記します。
  `Returns:` には戻り値の意味、特に `bool` や `None` の条件を記します。
  `Raises:` には利用者が対処すべき、確認済みの例外条件を記します。
  戻り値が `None` の場合、原則として `Returns:` は不要です。
- 型注釈にある型を docstring へ繰り返しません。ジェネレーターには必要に応じて
  `Yields:`、公開データ属性には `Attributes:`、特殊な呼び出し方には短い
  `Examples:` を使います。
- Maya の操作が実行される時点、Undo、既存ノードへの影響、UI signal の発火条件や
  破棄時の扱いなど、利用時の判断に必要な副作用を優先して説明します。
- コード内コメントは、処理のまとまりごとに意図、順序、制約を短く説明します。
  既存の一行コメントも含め、冗長または実装と異なる記述は統合・修正します。
- 内部実装では、名前とコードから分かりにくい振る舞いを中心に記述します。
  Maya API の意味や例外を推測で断定せず、必要なら `mayapy` で確認します。
- `_generated*`、`node_attr*` と生成由来の薄いノードクラスには、個別の日本語説明や
  コメントを追加しません。生成ファイルを手編集せず、手書きの拡張メソッドには
  通常どおり docstring を記述します。
- `.pyi` を通る公開 API は、実装側の説明が IDE の hover と補完に表示されるかを
  確認します。生成 `.pyi` は直接編集しません。
