...
固定2入力の `bdDbl3_Mult` と、可変長入力の `bdDbl3_MultMulti` を
`Nodes` から使用できます。
...
`bdDbl3_Mult` の `input1` と `input2` は `double3` です。2入力を成分ごとに
乗算します。

任意個の `double3` を乗算する場合は `bdDbl3_MultMulti` の multi attribute
...
- v1.0.0 未満では将来の設計と使いやすさを優先し、互換性維持よりも改善を選んで、必要な破壊的変更を積極的に行います。安定した API 互換性の提供は v1.0.0 以降を対象とします。
