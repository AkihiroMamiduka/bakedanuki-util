...
固定2入力の `bdDbl3Mult` と、可変長入力の `bdDbl3MultMulti` を
`Nodes` から使用できます。
...
`bdDbl3Mult` の `input1` と `input2` は `double3` です。2入力を成分ごとに
乗算します。

任意個の `double3` を乗算する場合は `bdDbl3MultMulti` の multi attribute
