...
from ....attr.define.node_attr_maya2027.shot import ColorField
...
class GeneratedShot(DG):
    ...
    thumbnail = DataStringField()
    tn = thumbnail

    startFrame = TimeField(default_value=0.0)
    sf = startFrame

    endFrame = TimeField(default_value=24.0)
    ef = endFrame
    ...
    sequenceEndFrame = TimeField(default_value=24.0, writable=False)
    se = sequenceEndFrame
    ...
    labels = MessageField(multi=True, readable=False)
    lab = labels

    colorIndex = ShortField(default_value=0)
    coli = colorIndex
