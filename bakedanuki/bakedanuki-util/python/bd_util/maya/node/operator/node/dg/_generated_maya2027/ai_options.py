...
from ....attr.define.node_attr_maya2027.ai_options import (
...
class ProceduralInstancingOptimizationEnumPlugOperator(
    EnumPlugOperator["ProceduralInstancingOptimizationEnumAttrOperator"]
):
    __slots__ = ()
...
class ProceduralInstancingOptimizationEnumAttrOperator(
    EnumAttrOperator[ProceduralInstancingOptimizationEnumPlugOperator]
):
    __slots__ = ()
...
class ProceduralInstancingOptimizationEnumField(
    EnumField[
        ProceduralInstancingOptimizationEnumAttrOperator,
        ProceduralInstancingOptimizationEnumPlugOperator,
    ]
):
    __slots__ = ()

    ATTR_CLS = ProceduralInstancingOptimizationEnumAttrOperator
    PLUG_CLS = ProceduralInstancingOptimizationEnumPlugOperator
...
class GeneratedAiOptions(DG):
    ...
    stochasticVolumeInterpolation = BoolField(default_value=True)
    stochastic_volume_interpolation = stochasticVolumeInterpolation
    ...
    nitsPerUnit = FloatField(default_value=1000.0)
    nits_per_unit = nitsPerUnit
    ...
    lightSamples = LongField(
        default_value=4, min_value=0, max_value=1024, soft_max_value=128
    )
    ...
    proceduralInstancingOptimization = (
        ProceduralInstancingOptimizationEnumField(default_value=1)
    )
    procedural_instancing_optimization = proceduralInstancingOptimization
    ...
    subdivFrustumPadding = FloatField(default_value=0.0, min_value=0.0)
    ...
    ignoreMotionTransform = BoolField(default_value=False)
    ignore_motion_transform = ignoreMotionTransform

    ignoreMotionDeform = BoolField(default_value=False)
    ignore_motion_deform = ignoreMotionDeform
    ...
    stats_mode = Stats_modeEnumField(default_value=0)
    statsm = stats_mode

    report_enable = BoolField(default_value=False)
    reporte = report_enable

    report_file = DataStringField()
    reportf = report_file
    ...
    asset_searchpath = DataStringField()
    aspath = asset_searchpath
    ...
    absoluteAssetPaths = BoolField(default_value=True)
    absolute_asset_paths = absoluteAssetPaths
    ...
    exportRelativeOutputPaths = BoolField(default_value=False)
    export_relative_output_paths = exportRelativeOutputPaths
