...
from ....attr.define.node_attr.mash_maths import (
    FalloffObjectField,
    MColourField,
)
...
class _GeneratedMASH_Maths(DG):
    __slots__ = ()

    NODE_TYPE = "MASH_Maths"
    ...
    overwrite = BoolField(default_value=False)
    ...
    formula = DataStringField()

    formulaX = DataStringField()

    formulaY = DataStringField()

    formulaZ = DataStringField()

    falloffMessage = MessageField()
    fmsg = falloffMessage

    enableAll = BoolField(default_value=True)

    enableX = BoolField(default_value=False)

    enableY = BoolField(default_value=False)
