...
from .....attr.define.node_attr_maya2027.maya_usd_proxy_shape import (
...
class GeneratedMayaUsdProxyShape(Shape):
    __slots__ = ()

    NODE_TYPE = "mayaUsdProxyShape"
    ...
    filePathRelative = BoolField(default_value=False, writable=False)
    fpr = filePathRelative

    primPath = DataStringField()
    pp = primPath

    excludePrimPaths = DataStringField()
    epp = excludePrimPaths

    loadPayloads = BoolField(default_value=True, readable=False)
    lpl = loadPayloads

    shareStage = BoolField(default_value=True, readable=False)
    scmp = shareStage

    time = TimeField(default_value=0.0)
    tm = time

    complexity = LongField(
        default_value=0, min_value=0, max_value=8, soft_max_value=4
    )
    cplx = complexity

    inStageData = TypedField(readable=False)
    id = inStageData

    inStageDataCached = TypedField(writable=False)
    idc = inStageDataCached

    stageCacheId = LongField(default_value=-1)
    stcid = stageCacheId

    drawRenderPurpose = BoolField(default_value=False, readable=False)
    drp = drawRenderPurpose

    drawProxyPurpose = BoolField(default_value=True, readable=False)
    dpp = drawProxyPurpose

    drawGuidePurpose = BoolField(default_value=False, readable=False)
    dgp = drawGuidePurpose

    variantFallbacks = DataStringField()
    vfs = variantFallbacks

    outTime = TimeField(default_value=0.0, writable=False)
    otm = outTime

    outStageData = TypedField(writable=False)
    od = outStageData
    ...
    outStageSessionLayerId = DataStringField()
    oslid = outStageSessionLayerId

    outStageRootLayerId = DataStringField()
    orlid = outStageRootLayerId

    mutedLayers = DataStringArrayField()
    mla = mutedLayers

    lockedLayers = DataStringArrayField()
    lockla = lockedLayers

    layerManager = DataStringField()
    lymgr = layerManager

    recomputeLayers = LongLongIntField(default_value=0)
    rcpl = recomputeLayers

    useTargetedLayerInProxyAccessor = BoolField(default_value=False)
    utlpa = useTargetedLayerInProxyAccessor
    ...
    overrides = DataStringField(multi=True, category="arnold")

    overrideReceiveShadows = BoolField(default_value=False, category="arnold")

    overrideDoubleSided = BoolField(default_value=False, category="arnold")

    overrideSelfShadows = BoolField(default_value=False, category="arnold")

    overrideOpaque = BoolField(default_value=False, category="arnold")

    overrideMatte = BoolField(default_value=False, category="arnold")

    shareUsdStage = BoolField(default_value=True, category="arnold")

    aiInteractive = BoolField(default_value=True, category="arnold")

    aiLegacyTranslation = BoolField(default_value=False, category="arnold")
