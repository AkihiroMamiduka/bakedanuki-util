...
class _GeneratedHIKSkeletonGeneratorNode(DG):
    ...
    SpineCount = LongField(default_value=3)

    NeckCount = LongField(default_value=1)

    ShoulderCount = LongField(default_value=1)

    WantUpperArmRollBone = BoolField(default_value=False)

    WantLowerArmRollBone = BoolField(default_value=False)

    WantUpperLegRollBone = BoolField(default_value=False)

    WantLowerLegRollBone = BoolField(default_value=False)

    NbUpperArmRollBones = LongField(default_value=0)

    NbLowerArmRollBones = LongField(default_value=0)

    NbUpperLegRollBones = LongField(default_value=0)

    NbLowerLegRollBones = LongField(default_value=0)

    WantIndexFinger = BoolField(default_value=False)

    WantMiddleFinger = BoolField(default_value=False)

    WantRingFinger = BoolField(default_value=False)

    WantPinkyFinger = BoolField(default_value=False)

    WantThumb = BoolField(default_value=False)

    WantExtraFinger = BoolField(default_value=False)

    WantFingerBase = BoolField(default_value=False)

    WantInHandJoint = BoolField(default_value=False)

    FingerJointCount = LongField(default_value=3)

    WantIndexToe = BoolField(default_value=False)

    WantMiddleToe = BoolField(default_value=False)

    WantRingToe = BoolField(default_value=False)

    WantPinkyToe = BoolField(default_value=False)

    WantBigToe = BoolField(default_value=False)

    WantFootThumb = BoolField(default_value=False)

    WantToeBase = BoolField(default_value=False)

    WantInFootJoint = BoolField(default_value=False)

    ToeJointCount = LongField(default_value=3)

    WantHipsTranslation = BoolField(default_value=False)
