...
class TimingOutputEnumPlugOperator(
    EnumPlugOperator["TimingOutputEnumAttrOperator"]
):
    __slots__ = ()
...
class TimingOutputEnumAttrOperator(
    EnumAttrOperator[TimingOutputEnumPlugOperator]
):
    __slots__ = ()
...
class GeneratedNucleus(Transform):
    __slots__ = ()
    ...
    lastTime = TimeField(default_value=-1568600686539.928)
    lti = lastTime

    evalId = TimeField(default_value=-1568600686539.928)
    eid = evalId

    skipSetup = BoolField(default_value=False)
    sksp = skipSetup

    startTime = TimeField(default_value=1.0)
    sti = startTime

    startFrame = DoubleField(default_value=1.0)
    stf = startFrame

    frameJumpLimit = LongField(
    ...
    windNoise = FloatField(
        default_value=0.0, soft_min_value=0.0, soft_max_value=1.0
    )
    wnoi = windNoise

    collisionLayerRange = FloatField(
        default_value=4.0, soft_min_value=1.0, soft_max_value=10.0
    )
    clra = collisionLayerRange

    collisionSoftness = FloatField(
    ...
    planeBounce = FloatField(
        default_value=0.0, soft_min_value=0.0, soft_max_value=1.0
    )
    npbc = planeBounce

    planeFriction = FloatField(
    ...
    maxCollisionIterations = LongField(
