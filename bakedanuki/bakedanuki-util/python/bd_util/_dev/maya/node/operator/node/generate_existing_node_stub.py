...
def collect_creatable_shape_definitions(
    ...
    tree = ast.parse(
        shape_types_path.read_text(encoding="utf-8"),
        filename=str(shape_types_path),
    )
    shape_node_types: frozenset[str] | None = None
    for statement in tree.body:
        if not isinstance(statement, ast.Assign):
            continue
        if not any(
            isinstance(target, ast.Name)
            and target.id == "CREATABLE_SHAPE_NODE_TYPES"
        ...
        if not all(isinstance(item, str) for item in literal_items):
            break
        shape_node_types = frozenset(cast(set[str], literal_items))
        break

    if shape_node_types is None:
        raise ValueError(
            "CREATABLE_SHAPE_NODE_TYPES definition not found or invalid"
        )

    definitions = tuple(
        definition
        for definition in collect_node_definitions(python_root)
        if definition.node_type in shape_node_types
        and definition.module_name.startswith(".operator.node.dag.shape.")
    )
    found_node_types = {definition.node_type for definition in definitions}
    missing_node_types = shape_node_types - found_node_types
    if missing_node_types:
        missing = ", ".join(sorted(missing_node_types))
        raise ValueError(f"Creatable shape definitions not found: {missing}")
    return definitions
