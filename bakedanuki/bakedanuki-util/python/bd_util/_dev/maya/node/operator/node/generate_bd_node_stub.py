...
def collect_node_definitions(python_root: Path) -> tuple[NodeDefinition, ...]:
    ...
    if len(method_names) != len(set(method_names)):
        raise ValueError("Duplicate BDNode method names found")
...
def generate_bd_node_stub_code(python_root: Path) -> str:
    definitions = collect_node_definitions(python_root)
    lines = [
        "# coding: utf-8",
        "# This file is generated by generate_bd_node_stub.py.",
        "from __future__ import annotations",
        "",
...
def bd_node_stub_path(python_root: Path) -> Path:
    return python_root / "bd_util" / "maya" / "node" / "bd_node.pyi"
...
def main() -> None:
    ...
    code = generate_bd_node_stub_code(python_root)

    if args.check:
        if not output_path.is_file() or output_path.read_text(
            encoding="utf-8"
        ) != code:
            raise SystemExit(f"BDNode stub is out of date: {output_path}")
        return
