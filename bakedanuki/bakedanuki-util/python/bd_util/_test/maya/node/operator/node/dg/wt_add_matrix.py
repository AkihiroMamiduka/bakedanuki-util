...
def class_instance_access():
    ...
    node = WtAddMatrix(modifier_manager, name=node_name)
...
def get_set():
    ...
    test_str.separator()
    node.wtMatrix[0].matrixIn.set_direct(node.matrixSum.get())
    logger.debug(
        "{}: {}".format(
    ...
    node.wtMatrix[0].weightIn.set(200)
    modifier_manager.do_it_dg()
    logger.debug(
        "{}: {}".format(
            "node.wtMatrix[0].weightIn.get()",
...
def connect_next_index():
    ...
    if cmds.objExists(node_name):
        cmds.delete(node_name)
    cmds.createNode("wtAddMatrix", name=node_name, skipSelect=True)
    dst_node = WtAddMatrix(modifier_manager, name=node_name)
    for i in range(5):
        node_name = f"src_{i}"
        if cmds.objExists(node_name):
            cmds.delete(node_name)
