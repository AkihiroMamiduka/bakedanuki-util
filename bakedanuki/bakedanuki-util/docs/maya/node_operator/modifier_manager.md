...
このAPIは`KeyframeManager.set_key()` / `set_keys()`のAPI経路と、挿入・tangent変更・
キー削除が使用します。`set_keys()`はバッチ内のAPI編集を1つの変更キャッシュへ記録します。
...
`KeyframeManager.set_key()` / `set_keys()`は、この入口で実行時のscene状態から
編集経路を選択します。cmdsへ委譲する場合は供給されたmodifierへcommandを予約し、API経路では後続の
`queue_anim_curve_change()`で編集します。これにより、作成から追加・上書きまでを
同じ実行境界へ積み、Undo / Redoでは初回に選択した経路の記録を再利用できます。
`set_keys()`の新規カーブでは最初のキーをcmdsで作成・反映した後、後続のキーをAPIで
扱えるか再判定します。cmdsへ委譲するキーは、それぞれ別のcommand callbackに記録し、
途中の失敗でも先行したキーの変更を同じ実行境界で戻します。
