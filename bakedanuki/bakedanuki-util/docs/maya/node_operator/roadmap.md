...
引数は予約時のUI時間単位で捕捉し、秒単位の長さを維持します。時間拡縮は後続候補です。
その後の着手順は未確定です。
...
| 候補 | 現状と、実装前に決めること |
| --- | --- |
| 移動の拡張 | 時間方向の初期版は実装済み。値方向の移動や時間の拡大縮小は未実装。layerの生値と合成後の値、接線変換の意味を個別に決める |
| キー削減の拡張・最適化 | 手動接線を維持する初期版は実装済み。より多くのキーを削減する探索方法、大規模カーブの性能改善、TT対応、現在保守的に残すweighted区間の判定拡張が候補。接線を削減のために調整する機能は初期版の方針に含めない |
| アニメーションライブラリー向けの一括操作 | `AnimationClip`の一括保存・復元と復元時刻指定は実装済み。時間拡縮やrig固有の属性対応・座標変換は未実装。汎用データ処理とrig固有処理の責務を分ける |
