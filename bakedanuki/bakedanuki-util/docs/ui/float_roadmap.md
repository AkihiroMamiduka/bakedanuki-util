...
2026-09-14時点で、単一値・3成分のMVVM基盤、ラベル・スライダー・範囲／step編集付きView、
Min／Max・stepの保存・復元まで、今回予定した機能実装は完了しています。
保存・復元の追加後もユーザーから動作確認・push完了の報告を受け、今回の開発を一旦完了としています。
...
| Min／Max・stepの保存・復元 | `UiStateManager`へ行を登録 | `UiStateManager`へXYZを独立登録 |
...
今回の合意範囲に未完了の実装はありません。次のリセット操作は今後の任意候補で、
実装予定として確定していません。必要性を確認してから、公開API名・UI配置を決めます。
...
実装時は保存データの削除に加え、表示中のViewと退避値も初期設定へ揃えてください。
`UiStateManager.clear()`の範囲と、既定値・保存形式を変更する際の注意点は
[保存機能の開発時の引き継ぎ](float_view_settings.md#開発時の引き継ぎ)にまとめています。
...
ユーザーの動作確認報告は、自動テストとは別の確認記録として扱います。
今回の報告では使用Maya versionや個別の操作条件までは記録していないため、
Maya再起動後の復元・実際のworkspaceControl操作などを全対応versionで確認済みとは扱いません。
