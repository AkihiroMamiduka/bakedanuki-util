...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDbl3Add.NODE_TYPE == "bdDbl3Add"
    ...
    assert BdDbl3AddMulti.NODE_TYPE == "bdDbl3AddMulti"
...
def test_defaults_return_additive_identity(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdDbl3Add()
    multi = nodes.create.bdDbl3AddMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDbl3Add1"
    assert multi.name == "bdDbl3AddMulti1"
...
def test_fixed_adds_two_inputs_component_wise(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl3Add(name="add")
...
def test_multi_adds_sparse_elements_and_updates_after_removal(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl3AddMulti(name="add")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDbl3Add")
        ...
        multi = maya_cmds.createNode("bdDbl3AddMulti")
...
def test_child_input_dependencies_cover_the_output_compound(
    ...
    for node_type, input_children in (
        ("bdDbl3Add", ("input1X", "input1Y", "input1Z")),
        ("bdDbl3AddMulti", ("inputX", "inputY", "inputZ")),
    ):
        node = maya_cmds.createNode(node_type)
...
def test_fixed_output_participates_in_multi_sum(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDbl3Add(name="fixed")
    multi = nodes.create.bdDbl3AddMulti(name="multi")
...
def test_existing_node_accessors_return_specific_wrappers(
    ...
    assert isinstance(nodes.existing.bdDbl3Add(fixed.name), BdDbl3Add)
    assert isinstance(
        nodes.existing.bdDbl3AddMulti(multi.name),
        BdDbl3AddMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    assert existing_nodes.existing.bdDbl3Add(
        "fixed"
    ).output.get().as_tuple() == pytest.approx(expected)
    assert existing_nodes.existing.bdDbl3AddMulti(
        "multi"
    ).output.get().as_tuple() == pytest.approx(expected)
