...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdSubDoublePair.NODE_TYPE == "bdSubDoublePair"
    assert BdSubDoublePair.input1.long_name == "input1"
    assert BdSubDoublePair.i1.short_name == "i1"
    assert BdSubDoublePair.input2.long_name == "input2"
    assert BdSubDoublePair.i2.short_name == "i2"
    assert BdSubDoublePair.output.long_name == "output"
    assert BdSubDoublePair.o.short_name == "o"

    assert BdSubDoubleMulti.NODE_TYPE == "bdSubDoubleMulti"
    assert BdSubDoubleMulti.input.long_name == "input"
    assert BdSubDoubleMulti.i.short_name == "i"
    assert BdSubDoubleMulti.output.long_name == "output"
    assert BdSubDoubleMulti.o.short_name == "o"
...
def test_defaults_and_fixed_subtraction(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdSubDoublePair()
    multi = nodes.create.bdSubDoubleMulti()
    fixed.input1.set(10.0)
    fixed.input2.set(3.5)
    modifier_manager.do_it_dg()

    assert fixed.name == "bdSubDoublePair1"
    assert multi.name == "bdSubDoubleMulti1"
...
def test_multi_uses_logical_index_order_and_defined_edge_cases(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdSubDoubleMulti(name="sub")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdSubDoublePair")
        ...
        multi = maya_cmds.createNode("bdSubDoubleMulti")
...
def test_connection_and_existing_node_accessors(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdSubDoublePair(name="fixed")
    multi = nodes.create.bdSubDoubleMulti(name="multi")
    fixed.input1.set(10.0)
    ...
    assert isinstance(
        nodes.existing.bdSubDoublePair(fixed.name), BdSubDoublePair
    )
    assert isinstance(
        nodes.existing.bdSubDoubleMulti(multi.name),
        BdSubDoubleMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_sub_double_pair.ma"
    ...
    assert existing_nodes.existing.bdSubDoublePair(
        "fixed"
    ).output.get() == pytest.approx(7.0)
    assert existing_nodes.existing.bdSubDoubleMulti(
        "multi"
