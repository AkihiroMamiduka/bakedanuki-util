...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDoubleAdd.NODE_TYPE == "bdDoubleAdd"
    assert BdDoubleAdd.input1.long_name == "input1"
    assert BdDoubleAdd.i1.short_name == "i1"
    assert BdDoubleAdd.input2.long_name == "input2"
    assert BdDoubleAdd.i2.short_name == "i2"
    assert BdDoubleAdd.output.long_name == "output"
    assert BdDoubleAdd.o.short_name == "o"

    assert BdDoubleAddMulti.NODE_TYPE == "bdDoubleAddMulti"
    assert BdDoubleAddMulti.input.long_name == "input"
    assert BdDoubleAddMulti.i.short_name == "i"
    assert BdDoubleAddMulti.output.long_name == "output"
    assert BdDoubleAddMulti.o.short_name == "o"
...
def test_defaults_return_additive_identity(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdDoubleAdd()
    multi = nodes.create.bdDoubleAddMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDoubleAdd1"
    assert multi.name == "bdDoubleAddMulti1"
...
def test_fixed_adds_two_inputs(modifier_manager, maya_cmds):
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDoubleAdd(name="add")
    node.input1.set(-2.5)
    node.input2.set(4.0)
    modifier_manager.do_it_dg()
...
def test_multi_adds_sparse_elements_and_updates_after_removal(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDoubleAddMulti(name="add")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDoubleAdd")
        ...
        multi = maya_cmds.createNode("bdDoubleAddMulti")
...
def test_fixed_output_participates_in_multi_sum(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDoubleAdd(name="fixed")
    multi = nodes.create.bdDoubleAddMulti(name="multi")
...
def test_existing_node_accessors_return_specific_wrappers(
    ...
    assert isinstance(nodes.existing.bdDoubleAdd(fixed.name), BdDoubleAdd)
    assert isinstance(
        nodes.existing.bdDoubleAddMulti(multi.name),
        BdDoubleAddMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_double_add.ma"
    ...
    assert existing_nodes.existing.bdDoubleAdd(
        "fixed"
    ).output.get() == pytest.approx(7.0)
    assert existing_nodes.existing.bdDoubleAddMulti(
        "multi"
    ).output.get() == pytest.approx(7.0)
