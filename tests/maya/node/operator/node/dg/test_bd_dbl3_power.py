...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDbl3Power.NODE_TYPE == "bdDbl3_Power"
    assert BdDbl3Power.input1.long_name == "input1"
    assert BdDbl3Power.i1x.short_name == "i1x"
    assert BdDbl3Power.input2.long_name == "input2"
    assert BdDbl3Power.i2z.short_name == "i2z"
    assert BdDbl3Power.output.long_name == "output"
    assert BdDbl3Power.oz.short_name == "oz"

    assert BdDbl3PowerMulti.NODE_TYPE == "bdDbl3_PowerMulti"
    assert BdDbl3PowerMulti.input.long_name == "input"
    assert BdDbl3PowerMulti.i.short_name == "i"
    assert BdDbl3PowerMulti.output.long_name == "output"
    assert BdDbl3PowerMulti.oz.short_name == "oz"
...
def test_defaults_and_fixed_component_wise_power(
    ...
    fixed = nodes.create.bdDbl3_Power()
    multi = nodes.create.bdDbl3_PowerMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDbl3_Power1"
    assert multi.name == "bdDbl3_PowerMulti1"
...
def test_fixed_clamps_small_bases_per_negative_exponent_component(maya_cmds):
    ...
    node = maya_cmds.createNode("bdDbl3_Power")
    maya_cmds.setAttr(
        f"{node}.input1",
        0.0,
        0.0,
...
def test_multi_uses_logical_index_order_and_defined_edge_cases(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl3_PowerMulti(name="pow")
...
def test_multi_clamps_current_bases_per_negative_exponent_component(maya_cmds):
    ...
    node = maya_cmds.createNode("bdDbl3_PowerMulti")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDbl3_Power")
        ...
        multi = maya_cmds.createNode("bdDbl3_PowerMulti")
...
def test_child_input_dependencies_cover_the_output_compound(
    ...
    for node_type, input_children in (
        ...
        ("bdDbl3_PowerMulti", ("inputX", "inputY", "inputZ")),
    ):
        node = maya_cmds.createNode(node_type)
...
def test_connection_and_existing_node_accessors(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDbl3_Power(name="fixed")
    multi = nodes.create.bdDbl3_PowerMulti(name="multi")
    fixed.input1.set((2.0, 3.0, 4.0))
    fixed.input2.set((3.0, 2.0, 0.5))
    ...
    assert isinstance(nodes.existing.bdDbl3_Power(fixed.name), BdDbl3Power)
    assert isinstance(
        nodes.existing.bdDbl3_PowerMulti(multi.name),
        BdDbl3PowerMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    assert fixed_fn.typeId.id() == 0x00142691
    assert multi_fn.typeId.id() == 0x00142690

    scene_path = tmp_path / "bd_dbl3_power.ma"
    ...
    assert existing_nodes.existing.bdDbl3_Power(
        "fixed"
    ).output.get().as_tuple() == pytest.approx((8.0, 9.0, 2.0))
    assert existing_nodes.existing.bdDbl3_PowerMulti(
        "multi"
