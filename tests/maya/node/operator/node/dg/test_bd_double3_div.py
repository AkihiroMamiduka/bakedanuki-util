...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDouble3Div.NODE_TYPE == "bdDouble3Div"
    assert BdDouble3Div.input1.long_name == "input1"
    assert BdDouble3Div.i1x.short_name == "i1x"
    assert BdDouble3Div.input2.long_name == "input2"
    assert BdDouble3Div.i2z.short_name == "i2z"
    assert BdDouble3Div.output.long_name == "output"
    assert BdDouble3Div.oz.short_name == "oz"

    assert BdDouble3DivMulti.NODE_TYPE == "bdDouble3DivMulti"
    assert BdDouble3DivMulti.input.long_name == "input"
    assert BdDouble3DivMulti.i.short_name == "i"
    assert BdDouble3DivMulti.output.long_name == "output"
    assert BdDouble3DivMulti.oz.short_name == "oz"
...
def test_defaults_and_fixed_component_wise_division(
    ...
    fixed = nodes.create.bdDouble3Div()
    multi = nodes.create.bdDouble3DivMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDouble3Div1"
    assert multi.name == "bdDouble3DivMulti1"
...
def test_fixed_clamps_small_divisors_per_component(maya_cmds):
    ...
    node = maya_cmds.createNode("bdDouble3Div")
...
def test_multi_uses_logical_index_order_and_defined_edge_cases(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDouble3DivMulti(name="div")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDouble3Div")
        ...
        multi = maya_cmds.createNode("bdDouble3DivMulti")
...
def test_child_input_dependencies_cover_the_output_compound(
    ...
    for node_type, input_children in (
        ...
        ("bdDouble3DivMulti", ("inputX", "inputY", "inputZ")),
    ):
        node = maya_cmds.createNode(node_type)
...
def test_connection_and_existing_node_accessors(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDouble3Div(name="fixed")
    multi = nodes.create.bdDouble3DivMulti(name="multi")
    fixed.input1.set((12.0, 20.0, 30.0))
    fixed.input2.set((3.0, 4.0, 5.0))
    ...
    assert isinstance(nodes.existing.bdDouble3Div(fixed.name), BdDouble3Div)
    assert isinstance(
        nodes.existing.bdDouble3DivMulti(multi.name),
        BdDouble3DivMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_double3_div.ma"
    ...
    assert existing_nodes.existing.bdDouble3Div(
        "fixed"
    ).output.get().as_tuple() == pytest.approx((4.0, 5.0, 6.0))
    assert existing_nodes.existing.bdDouble3DivMulti(
        "multi"
