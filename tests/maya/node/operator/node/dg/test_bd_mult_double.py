...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdMultDoublePair.NODE_TYPE == "bdMultDoublePair"
    assert BdMultDoublePair.input1.long_name == "input1"
    assert BdMultDoublePair.i1.short_name == "i1"
    assert BdMultDoublePair.input2.long_name == "input2"
    assert BdMultDoublePair.i2.short_name == "i2"
    assert BdMultDoublePair.output.long_name == "output"
    assert BdMultDoublePair.o.short_name == "o"

    assert BdMultDoubleMulti.NODE_TYPE == "bdMultDoubleMulti"
    assert BdMultDoubleMulti.input.long_name == "input"
    assert BdMultDoubleMulti.i.short_name == "i"
    assert BdMultDoubleMulti.output.long_name == "output"
    assert BdMultDoubleMulti.o.short_name == "o"
...
def test_default_names_and_values(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdMultDoublePair()
    multi = nodes.create.bdMultDoubleMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdMultDoublePair1"
    assert multi.name == "bdMultDoubleMulti1"
...
def test_fixed_multiplies_two_inputs(modifier_manager, maya_cmds):
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdMultDoublePair(name="mult")
    node.input1.set(-2.5)
    node.input2.set(4.0)
    modifier_manager.do_it_dg()
...
def test_multi_multiplies_existing_sparse_elements(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdMultDoubleMulti(name="mult")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdMultDoublePair")
        ...
        multi = maya_cmds.createNode("bdMultDoubleMulti")
...
def test_fixed_output_participates_in_multi_product(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdMultDoublePair(name="fixed")
    multi = nodes.create.bdMultDoubleMulti(name="multi")
...
def test_existing_node_accessors_return_specific_wrappers(
    ...
    assert isinstance(
        nodes.existing.bdMultDoublePair(fixed.name), BdMultDoublePair
    )
    assert isinstance(
        nodes.existing.bdMultDoubleMulti(multi.name),
        BdMultDoubleMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_mult_double_pair.ma"
    ...
    assert existing_nodes.existing.bdMultDoublePair(
        "fixed"
    ).output.get() == pytest.approx(10.0)
    assert existing_nodes.existing.bdMultDoubleMulti(
        "multi"
    ).output.get() == pytest.approx(10.0)
