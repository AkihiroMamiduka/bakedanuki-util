...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDoubleDiv.NODE_TYPE == "bdDoubleDiv"
    assert BdDoubleDiv.input1.long_name == "input1"
    assert BdDoubleDiv.i1.short_name == "i1"
    assert BdDoubleDiv.input2.long_name == "input2"
    assert BdDoubleDiv.i2.short_name == "i2"
    assert BdDoubleDiv.output.long_name == "output"
    assert BdDoubleDiv.o.short_name == "o"

    assert BdDoubleDivMulti.NODE_TYPE == "bdDoubleDivMulti"
    assert BdDoubleDivMulti.input.long_name == "input"
    assert BdDoubleDivMulti.i.short_name == "i"
    assert BdDoubleDivMulti.output.long_name == "output"
    assert BdDoubleDivMulti.o.short_name == "o"
...
def test_defaults_and_fixed_division(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdDoubleDiv()
    multi = nodes.create.bdDoubleDivMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDoubleDiv1"
    assert multi.name == "bdDoubleDivMulti1"
...
def test_fixed_clamps_small_divisors_with_sign(
    ...
    node = maya_cmds.createNode("bdDoubleDiv")
...
def test_multi_uses_logical_index_order_and_defined_edge_cases(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDoubleDivMulti(name="div")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDoubleDiv")
        ...
        multi = maya_cmds.createNode("bdDoubleDivMulti")
...
def test_connection_and_existing_node_accessors(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDoubleDiv(name="fixed")
    multi = nodes.create.bdDoubleDivMulti(name="multi")
    fixed.input1.set(12.0)
    fixed.input2.set(3.0)
    ...
    assert isinstance(nodes.existing.bdDoubleDiv(fixed.name), BdDoubleDiv)
    assert isinstance(
        nodes.existing.bdDoubleDivMulti(multi.name),
        BdDoubleDivMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_double_div.ma"
    ...
    assert existing_nodes.existing.bdDoubleDiv(
        "fixed"
    ).output.get() == pytest.approx(4.0)
    assert existing_nodes.existing.bdDoubleDivMulti(
        "multi"
