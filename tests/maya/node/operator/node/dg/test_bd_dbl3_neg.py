...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    assert BdDbl3Neg.NODE_TYPE == "bdDbl3_Neg"
    assert BdDbl3Neg.input.long_name == "input"
    assert BdDbl3Neg.ix.short_name == "ix"
    assert BdDbl3Neg.iy.short_name == "iy"
    assert BdDbl3Neg.iz.short_name == "iz"
    assert BdDbl3Neg.output.long_name == "output"
    assert BdDbl3Neg.ox.short_name == "ox"
    assert BdDbl3Neg.oy.short_name == "oy"
    assert BdDbl3Neg.oz.short_name == "oz"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl3_Neg()
...
def test_negates_each_component(maya_cmds):
    ...
    node = maya_cmds.createNode("bdDbl3_Neg")
...
def test_connections_existing_accessor_and_scene_round_trip(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    source = nodes.create.bdDbl3_Neg(name="source_neg3")
    target = nodes.create.bdDbl3_Neg(name="target_neg3")
    ...
    existing = nodes.existing.bdDbl3_Neg(target.name)
    assert type(existing) is type(target)

    scene_path = tmp_path / "bd_dbl3_neg.ma"
    ...
    output = reloaded.existing.bdDbl3_Neg("target_neg3").output.get()
