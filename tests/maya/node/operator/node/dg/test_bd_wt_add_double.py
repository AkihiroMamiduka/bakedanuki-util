...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    from bd_util.maya.node.operator.node.dg.bd_wt_add_double_multi import (
        BdWtAddDoubleMulti,
    )

    assert BdWtAddDoubleMulti.NODE_TYPE == "bdWtAddDoubleMulti"
    assert BdWtAddDoubleMulti.input.long_name == "input"
    assert BdWtAddDoubleMulti.i.short_name == "i"
    assert BdWtAddDoubleMulti.output.long_name == "output"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdWtAddDoubleMulti()
...
def test_sums_sparse_weighted_values_without_normalizing(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdWtAddDoubleMulti(name="weighted")
...
def test_child_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        node = maya_cmds.createNode("bdWtAddDoubleMulti")
