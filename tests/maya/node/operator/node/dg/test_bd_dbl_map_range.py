...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    assert BdDblMapRange.sourceMinimum.long_name == "sourceMinimum"
    assert BdDblMapRange.smin.short_name == "smin"
    assert BdDblMapRange.sourceMaximum.long_name == "sourceMaximum"
    assert BdDblMapRange.smax.short_name == "smax"
    assert BdDblMapRange.targetMinimum.long_name == "targetMinimum"
    assert BdDblMapRange.tmin.short_name == "tmin"
    assert BdDblMapRange.targetMaximum.long_name == "targetMaximum"
    assert BdDblMapRange.tmax.short_name == "tmax"
    ...
    assert node.sourceMinimum.get() == pytest.approx(0.0)
    assert node.sourceMaximum.get() == pytest.approx(1.0)
    assert node.targetMinimum.get() == pytest.approx(0.0)
    assert node.targetMaximum.get() == pytest.approx(1.0)
...
def test_maps_directional_ranges_and_optional_extrapolation(
    ...
    maya_cmds.setAttr(f"{node}.sourceMinimum", source_minimum)
    maya_cmds.setAttr(f"{node}.sourceMaximum", source_maximum)
    maya_cmds.setAttr(f"{node}.targetMinimum", target_minimum)
    maya_cmds.setAttr(f"{node}.targetMaximum", target_maximum)
...
def test_zero_width_source_returns_target_minimum(
    ...
    maya_cmds.setAttr(f"{node}.sourceMinimum", source_minimum)
    maya_cmds.setAttr(f"{node}.sourceMaximum", source_maximum)
    maya_cmds.setAttr(f"{node}.targetMinimum", -3.0)
    maya_cmds.setAttr(f"{node}.targetMaximum", 7.0)
...
def test_nan_propagates_from_each_numeric_input(maya_cmds, nan_attribute):
    ...
    maya_cmds.setAttr(f"{node}.sourceMinimum", 0.0)
    maya_cmds.setAttr(f"{node}.sourceMaximum", 1.0)
    maya_cmds.setAttr(f"{node}.targetMinimum", 0.0)
    maya_cmds.setAttr(f"{node}.targetMaximum", 1.0)
...
def test_infinite_input_uses_clamp_or_extrapolation(
    ...
    maya_cmds.setAttr(f"{node}.sourceMinimum", 0.0)
    maya_cmds.setAttr(f"{node}.sourceMaximum", 1.0)
    maya_cmds.setAttr(f"{node}.targetMinimum", -5.0)
    maya_cmds.setAttr(f"{node}.targetMaximum", 5.0)
    maya_cmds.setAttr(f"{node}.clamp", should_clamp)
...
def test_infinite_target_endpoints_are_exact_and_midpoint_is_nan(maya_cmds):
    ...
    maya_cmds.setAttr(f"{node}.sourceMinimum", 0.0)
    maya_cmds.setAttr(f"{node}.sourceMaximum", 1.0)
    maya_cmds.setAttr(f"{node}.targetMinimum", float("-inf"))
    maya_cmds.setAttr(f"{node}.targetMaximum", float("inf"))
...
def test_target_endpoint_signed_zero_is_preserved(maya_cmds):
    ...
    maya_cmds.setAttr(f"{node}.sourceMaximum", 1.0)
    maya_cmds.setAttr(f"{node}.targetMinimum", -0.0)
    maya_cmds.setAttr(f"{node}.targetMaximum", 0.0)
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        maya_cmds.setAttr(f"{node}.sourceMinimum", 0.0)
        maya_cmds.setAttr(f"{node}.sourceMaximum", 10.0)
        maya_cmds.setAttr(f"{node}.targetMinimum", 0.0)
        maya_cmds.setAttr(f"{node}.targetMaximum", 10.0)
        ...
        maya_cmds.setAttr(f"{node}.sourceMinimum", 20.0)
        maya_cmds.setAttr(f"{node}.sourceMaximum", 0.0)
...
def test_connections_existing_accessor_and_scene_round_trip(
    ...
    source.sourceMinimum.set(0.0)
    source.sourceMaximum.set(10.0)
    source.output.connect(target.input)
    target.targetMinimum.set(10.0)
    target.targetMaximum.set(20.0)
    modifier_manager.do_it_dg()
