...
def test_class_attribute_access_and_default(modifier_manager, maya_cmds):
    ...
    assert BdDbl3Value.NODE_TYPE == "bdDbl3Value"
    ...
    node = nodes.create.bdDbl3Value()
    modifier_manager.do_it_dg()

    assert node.name == "bdDbl3Value1"
...
def test_value_parent_and_children_are_editable(maya_cmds):
    ...
    node = maya_cmds.createNode("bdDbl3Value")
...
def test_value_can_be_destination_and_source_in_the_same_network(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    source = nodes.create.bdDbl3Value(name="source")
    relay = nodes.create.bdDbl3Value(name="relay")
    mult = nodes.create.bdDbl3Mult(name="mult")
...
def test_child_value_can_drive_scalar_plug(modifier_manager, maya_cmds):
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    value = nodes.create.bdDbl3Value(name="value")
...
def test_downstream_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        value = maya_cmds.createNode("bdDbl3Value")
        mult = maya_cmds.createNode("bdDbl3Mult")
...
def test_existing_accessor_and_scene_round_trip(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl3Value(name="value")
    node.value.set((1.5, 2.5, 3.5))
    modifier_manager.do_it_dg()

    assert isinstance(nodes.existing.bdDbl3Value(node.name), BdDbl3Value)
    ...
    assert existing_nodes.existing.bdDbl3Value(
        "value"
