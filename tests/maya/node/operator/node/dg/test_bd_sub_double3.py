...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdSubDouble3Pair.NODE_TYPE == "bdSubDouble3Pair"
    assert BdSubDouble3Pair.input1.long_name == "input1"
    assert BdSubDouble3Pair.i1x.short_name == "i1x"
    assert BdSubDouble3Pair.input2.long_name == "input2"
    assert BdSubDouble3Pair.i2z.short_name == "i2z"
    assert BdSubDouble3Pair.output.long_name == "output"
    assert BdSubDouble3Pair.oz.short_name == "oz"

    assert BdSubDouble3Multi.NODE_TYPE == "bdSubDouble3Multi"
    assert BdSubDouble3Multi.input.long_name == "input"
    assert BdSubDouble3Multi.i.short_name == "i"
    assert BdSubDouble3Multi.output.long_name == "output"
    assert BdSubDouble3Multi.oz.short_name == "oz"
...
def test_defaults_and_fixed_component_wise_subtraction(
    ...
    fixed = nodes.create.bdSubDouble3Pair()
    multi = nodes.create.bdSubDouble3Multi()
    fixed.input1.set((10.0, 20.0, 30.0))
    fixed.input2.set((1.0, 5.0, 7.0))
    modifier_manager.do_it_dg()

    assert fixed.name == "bdSubDouble3Pair1"
    assert multi.name == "bdSubDouble3Multi1"
...
def test_multi_uses_logical_index_order_and_defined_edge_cases(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdSubDouble3Multi(name="sub")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdSubDouble3Pair")
        ...
        multi = maya_cmds.createNode("bdSubDouble3Multi")
...
def test_child_input_dependencies_cover_the_output_compound(
    ...
    for node_type, input_children in (
        ...
        ("bdSubDouble3Multi", ("inputX", "inputY", "inputZ")),
    ):
        node = maya_cmds.createNode(node_type)
...
def test_connection_and_existing_node_accessors(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdSubDouble3Pair(name="fixed")
    multi = nodes.create.bdSubDouble3Multi(name="multi")
    fixed.input1.set((10.0, 20.0, 30.0))
    fixed.input2.set((1.0, 5.0, 7.0))
    ...
    assert isinstance(
        nodes.existing.bdSubDouble3Pair(fixed.name), BdSubDouble3Pair
    )
    assert isinstance(
        nodes.existing.bdSubDouble3Multi(multi.name),
        BdSubDouble3Multi,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_sub_double3_pair.ma"
    ...
    assert existing_nodes.existing.bdSubDouble3Pair(
        "fixed"
    ).output.get().as_tuple() == pytest.approx((9.0, 15.0, 23.0))
    assert existing_nodes.existing.bdSubDouble3Multi(
        "multi"
