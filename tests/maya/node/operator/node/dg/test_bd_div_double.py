...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDivDoublePair.NODE_TYPE == "bdDivDoublePair"
    assert BdDivDoublePair.input1.long_name == "input1"
    assert BdDivDoublePair.i1.short_name == "i1"
    assert BdDivDoublePair.input2.long_name == "input2"
    assert BdDivDoublePair.i2.short_name == "i2"
    assert BdDivDoublePair.output.long_name == "output"
    assert BdDivDoublePair.o.short_name == "o"

    assert BdDivDoubleMulti.NODE_TYPE == "bdDivDoubleMulti"
    assert BdDivDoubleMulti.input.long_name == "input"
    assert BdDivDoubleMulti.i.short_name == "i"
    assert BdDivDoubleMulti.output.long_name == "output"
    assert BdDivDoubleMulti.o.short_name == "o"
...
def test_defaults_and_fixed_division(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdDivDoublePair()
    multi = nodes.create.bdDivDoubleMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDivDoublePair1"
    assert multi.name == "bdDivDoubleMulti1"
...
def test_fixed_clamps_small_divisors_with_sign(
    ...
    node = maya_cmds.createNode("bdDivDoublePair")
...
def test_multi_uses_logical_index_order_and_defined_edge_cases(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDivDoubleMulti(name="div")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDivDoublePair")
        ...
        multi = maya_cmds.createNode("bdDivDoubleMulti")
...
def test_connection_and_existing_node_accessors(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDivDoublePair(name="fixed")
    multi = nodes.create.bdDivDoubleMulti(name="multi")
    fixed.input1.set(12.0)
    fixed.input2.set(3.0)
    ...
    assert isinstance(
        nodes.existing.bdDivDoublePair(fixed.name), BdDivDoublePair
    )
    assert isinstance(
        nodes.existing.bdDivDoubleMulti(multi.name),
        BdDivDoubleMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_div_double_pair.ma"
    ...
    assert existing_nodes.existing.bdDivDoublePair(
        "fixed"
    ).output.get() == pytest.approx(4.0)
    assert existing_nodes.existing.bdDivDoubleMulti(
        "multi"
