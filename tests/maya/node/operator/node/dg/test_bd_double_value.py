...
def test_class_attribute_access_and_default(modifier_manager, maya_cmds):
    ...
    from bd_util.maya.node.operator.node.dg.bd_double_value import (
        BdDoubleValue,
    )

    assert BdDoubleValue.NODE_TYPE == "bdDoubleValue"
    assert BdDoubleValue.value.long_name == "value"
    assert BdDoubleValue.v.short_name == "v"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDoubleValue()
    modifier_manager.do_it_dg()

    assert node.name == "bdDoubleValue1"
...
def test_value_is_readable_writable_storable_and_keyable(maya_cmds):
    ...
    node = maya_cmds.createNode("bdDoubleValue")
...
def test_value_can_be_destination_and_source_in_the_same_network(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    source = nodes.create.bdDoubleValue(name="source")
    relay = nodes.create.bdDoubleValue(name="relay")
...
def test_downstream_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        value = maya_cmds.createNode("bdDoubleValue")
        mult = maya_cmds.createNode("bdMultDoublePair")
...
def test_existing_accessor_and_scene_round_trip(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDoubleValue(name="value")
    node.value.set(7.5)
    modifier_manager.do_it_dg()

    assert isinstance(nodes.existing.bdDoubleValue(node.name), BdDoubleValue)
    ...
    scene_path = tmp_path / "bd_double_value.ma"
    ...
    assert existing_nodes.existing.bdDoubleValue(
        "value"
