...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    from bd_util.maya.node.operator.node.dg.bd_dbl_weighted_sum_multi import (
        BdDblWeightedSumMulti,
    )

    assert BdDblWeightedSumMulti.NODE_TYPE == "bdDbl_WeightedSumMulti"
    assert BdDblWeightedSumMulti.input.long_name == "input"
    assert BdDblWeightedSumMulti.i.short_name == "i"
    assert BdDblWeightedSumMulti.output.long_name == "output"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl_WeightedSumMulti()
...
def test_sums_sparse_weighted_values_without_normalizing(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl_WeightedSumMulti(name="weighted")
...
def test_child_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        node = maya_cmds.createNode("bdDbl_WeightedSumMulti")
