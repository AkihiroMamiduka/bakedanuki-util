...
def test_fixed_class_attribute_access(maya_cmds):
    ...
    assert BdMultDouble3Pair.NODE_TYPE == "bdMultDouble3Pair"
    assert BdMultDouble3Pair.input1.long_name == "input1"
    assert BdMultDouble3Pair.i1.short_name == "i1"
    assert BdMultDouble3Pair.input1X.long_name == "input1X"
    assert BdMultDouble3Pair.i1x.short_name == "i1x"
    assert BdMultDouble3Pair.input2.long_name == "input2"
    assert BdMultDouble3Pair.i2.short_name == "i2"
    assert BdMultDouble3Pair.input2Z.long_name == "input2Z"
    assert BdMultDouble3Pair.i2z.short_name == "i2z"
    assert BdMultDouble3Pair.output.long_name == "output"
    assert BdMultDouble3Pair.oz.short_name == "oz"
...
def test_multi_class_attribute_access(maya_cmds):
    ...
    assert BdMultDouble3Multi.NODE_TYPE == "bdMultDouble3Multi"
    assert BdMultDouble3Multi.input.long_name == "input"
    assert BdMultDouble3Multi.i.short_name == "i"
    assert BdMultDouble3Multi.output.long_name == "output"
    assert BdMultDouble3Multi.oz.short_name == "oz"
...
def test_default_names_have_unambiguous_maya_indices(
    ...
    fixed = nodes.create.bdMultDouble3Pair()
    multi = nodes.create.bdMultDouble3Multi()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdMultDouble3Pair1"
    assert multi.name == "bdMultDouble3Multi1"
...
def test_fixed_defaults_return_multiplicative_identity(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdMultDouble3Pair(name="mult")
    modifier_manager.do_it_dg()
...
def test_multi_empty_input_returns_multiplicative_identity(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdMultDouble3Multi(name="mult")
    modifier_manager.do_it_dg()
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdMultDouble3Pair")
        maya_cmds.setAttr(
            f"{fixed}.input1",
            2.0,
            3.0,
        ...
        multi = maya_cmds.createNode("bdMultDouble3Multi")
        maya_cmds.setAttr(
            f"{multi}.input[2]",
            2.0,
            3.0,
...
def test_child_input_dependencies_cover_the_output_compound(
    ...
    for node_type, input_children in (
        ("bdMultDouble3Pair", ("input1X", "input1Y", "input1Z")),
        ("bdMultDouble3Multi", ("inputX", "inputY", "inputZ")),
    ):
        node = maya_cmds.createNode(node_type)
...
def test_existing_node_accessors_return_specific_wrappers(
    ...
    existing_fixed = nodes.existing.bdMultDouble3Pair(fixed.name)
    existing_multi = nodes.existing.bdMultDouble3Multi(multi.name)
    assert isinstance(existing_fixed, BdMultDouble3Pair)
    assert isinstance(existing_multi, BdMultDouble3Multi)
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_mult_double3_pair.ma"
    ...
    existing_fixed = existing_nodes.existing.bdMultDouble3Pair("fixed")
    existing_multi = existing_nodes.existing.bdMultDouble3Multi("multi")
