...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    node = nodes.create.bdDbl3_Clamp()
    ...
    assert node_fn.typeId.id() == 0x001426A2


def test_clamps_component_wise_with_normalized_bounds(maya_cmds):
    _load_bd_util_nodes(maya_cmds)
...
def test_infinity_uses_component_wise_normal_ordering(maya_cmds):
    _load_bd_util_nodes(maya_cmds)
    ...
    assert maya_cmds.getAttr(f"{node}.output")[0] == (
        3.0,
        5.0,
        -5.0,
    )
...
def test_signed_zero_is_selected_per_component(maya_cmds):
    _load_bd_util_nodes(maya_cmds)
...
def test_connections_existing_accessor_and_scene_round_trip(
    ...
    existing = nodes.existing.bdDbl3_Clamp(target.name)
    assert type(existing) is type(target)

    scene_path = tmp_path / "bd_dbl3_clamp.ma"
    ...
    output = reloaded.existing.bdDbl3_Clamp("target_clamp3").output.get()
