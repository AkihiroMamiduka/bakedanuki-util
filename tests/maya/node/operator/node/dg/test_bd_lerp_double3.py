...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    from bd_util.maya.node.operator.node.dg.bd_lerp_double3_pair import (
        BdLerpDouble3Pair,
    )

    assert BdLerpDouble3Pair.NODE_TYPE == "bdLerpDouble3Pair"
    assert BdLerpDouble3Pair.input1.long_name == "input1"
    assert BdLerpDouble3Pair.i1x.short_name == "i1x"
    assert BdLerpDouble3Pair.input2.long_name == "input2"
    assert BdLerpDouble3Pair.i2z.short_name == "i2z"
    assert BdLerpDouble3Pair.weight.long_name == "weight"
    assert BdLerpDouble3Pair.output.long_name == "output"
    assert BdLerpDouble3Pair.oy.short_name == "oy"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdLerpDouble3Pair()
    ...
    assert node_fn.typeId.id() == 0x0007F017
...
def test_interpolates_components_with_one_weight(modifier_manager, maya_cmds):
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdLerpDouble3Pair(name="lerp")
...
def test_connected_weight_is_clamped(
    ...
    node = maya_cmds.createNode("bdLerpDouble3Pair")
