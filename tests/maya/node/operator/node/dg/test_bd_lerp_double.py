...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    from bd_util.maya.node.operator.node.dg.bd_lerp_double_pair import (
        BdLerpDoublePair,
    )

    assert BdLerpDoublePair.NODE_TYPE == "bdLerpDoublePair"
    assert BdLerpDoublePair.input1.long_name == "input1"
    assert BdLerpDoublePair.i2.short_name == "i2"
    assert BdLerpDoublePair.weight.long_name == "weight"
    assert BdLerpDoublePair.w.short_name == "w"
    assert BdLerpDoublePair.output.long_name == "output"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdLerpDoublePair()
...
def test_interpolates_between_inputs(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdLerpDoublePair(name="lerp")
...
def test_connected_weight_is_clamped(
    ...
    node = maya_cmds.createNode("bdLerpDoublePair")
