...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    assert BdDblNeg.NODE_TYPE == "bdDbl_Neg"
    assert BdDblNeg.input.long_name == "input"
    assert BdDblNeg.i.short_name == "i"
    assert BdDblNeg.output.long_name == "output"
    assert BdDblNeg.o.short_name == "o"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl_Neg()
...
def test_negates_value(maya_cmds, input_value, expected):
    ...
    node = maya_cmds.createNode("bdDbl_Neg")
...
def test_connections_existing_accessor_and_scene_round_trip(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    source = nodes.create.bdDbl_Neg(name="source_neg")
    target = nodes.create.bdDbl_Neg(name="target_neg")
    source.input.set(-3.5)
    source.output.connect(target.input)
    modifier_manager.do_it_dg()

    assert target.output.get() == pytest.approx(-3.5)
    existing = nodes.existing.bdDbl_Neg(target.name)
    assert type(existing) is type(target)

    scene_path = tmp_path / "bd_dbl_neg.ma"
    ...
    assert reloaded.existing.bdDbl_Neg("target_neg").output.get() == (
        pytest.approx(-3.5)
    )
