...
def test_class_attribute_access(maya_cmds):
    ...
    from bd_util.maya.node.operator.node.dg.bd_double_mult import (
        BdDoubleMult,
    )
    ...
    assert BdDoubleMult.NODE_TYPE == "bdDoubleMult"
    assert BdDoubleMult.input1.long_name == "input1"
    assert BdDoubleMult.i1.short_name == "i1"
    assert BdDoubleMult.input2.long_name == "input2"
    assert BdDoubleMult.i2.short_name == "i2"
    assert BdDoubleMult.output.long_name == "output"
    assert BdDoubleMult.o.short_name == "o"

    assert BdDoubleMultMulti.NODE_TYPE == "bdDoubleMultMulti"
    assert BdDoubleMultMulti.input.long_name == "input"
    assert BdDoubleMultMulti.i.short_name == "i"
    assert BdDoubleMultMulti.output.long_name == "output"
    assert BdDoubleMultMulti.o.short_name == "o"
...
def test_default_names_and_values(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdDoubleMult()
    multi = nodes.create.bdDoubleMultMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDoubleMult1"
    assert multi.name == "bdDoubleMultMulti1"
...
def test_fixed_multiplies_two_inputs(modifier_manager, maya_cmds):
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDoubleMult(name="mult")
    node.input1.set(-2.5)
    node.input2.set(4.0)
    modifier_manager.do_it_dg()
...
def test_multi_multiplies_existing_sparse_elements(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDoubleMultMulti(name="mult")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDoubleMult")
        ...
        multi = maya_cmds.createNode("bdDoubleMultMulti")
...
def test_fixed_output_participates_in_multi_product(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDoubleMult(name="fixed")
    multi = nodes.create.bdDoubleMultMulti(name="multi")
...
def test_existing_node_accessors_return_specific_wrappers(
    ...
    assert isinstance(nodes.existing.bdDoubleMult(fixed.name), BdDoubleMult)
    assert isinstance(
        nodes.existing.bdDoubleMultMulti(multi.name),
        BdDoubleMultMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_double_mult.ma"
    ...
    assert existing_nodes.existing.bdDoubleMult(
        "fixed"
    ).output.get() == pytest.approx(10.0)
    assert existing_nodes.existing.bdDoubleMultMulti(
        "multi"
    ).output.get() == pytest.approx(10.0)
