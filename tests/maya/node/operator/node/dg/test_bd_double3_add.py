...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDouble3Add.NODE_TYPE == "bdDouble3Add"
    assert BdDouble3Add.input1.long_name == "input1"
    assert BdDouble3Add.i1x.short_name == "i1x"
    assert BdDouble3Add.input2.long_name == "input2"
    assert BdDouble3Add.i2z.short_name == "i2z"
    assert BdDouble3Add.output.long_name == "output"
    assert BdDouble3Add.oz.short_name == "oz"

    assert BdDouble3AddMulti.NODE_TYPE == "bdDouble3AddMulti"
    assert BdDouble3AddMulti.input.long_name == "input"
    assert BdDouble3AddMulti.i.short_name == "i"
    assert BdDouble3AddMulti.output.long_name == "output"
    assert BdDouble3AddMulti.oz.short_name == "oz"
...
def test_defaults_return_additive_identity(modifier_manager, maya_cmds):
    ...
    fixed = nodes.create.bdDouble3Add()
    multi = nodes.create.bdDouble3AddMulti()
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDouble3Add1"
    assert multi.name == "bdDouble3AddMulti1"
...
def test_fixed_adds_two_inputs_component_wise(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDouble3Add(name="add")
...
def test_multi_adds_sparse_elements_and_updates_after_removal(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDouble3AddMulti(name="add")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDouble3Add")
        ...
        multi = maya_cmds.createNode("bdDouble3AddMulti")
...
def test_child_input_dependencies_cover_the_output_compound(
    ...
    for node_type, input_children in (
        ("bdDouble3Add", ("input1X", "input1Y", "input1Z")),
        ("bdDouble3AddMulti", ("inputX", "inputY", "inputZ")),
    ):
        node = maya_cmds.createNode(node_type)
...
def test_fixed_output_participates_in_multi_sum(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDouble3Add(name="fixed")
    multi = nodes.create.bdDouble3AddMulti(name="multi")
...
def test_existing_node_accessors_return_specific_wrappers(
    ...
    assert isinstance(nodes.existing.bdDouble3Add(fixed.name), BdDouble3Add)
    assert isinstance(
        nodes.existing.bdDouble3AddMulti(multi.name),
        BdDouble3AddMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    scene_path = tmp_path / "bd_double3_add.ma"
    ...
    assert existing_nodes.existing.bdDouble3Add(
        "fixed"
    ).output.get().as_tuple() == pytest.approx(expected)
    assert existing_nodes.existing.bdDouble3AddMulti(
        "multi"
    ).output.get().as_tuple() == pytest.approx(expected)
