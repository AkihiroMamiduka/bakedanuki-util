...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    assert BdDbl3Negate.NODE_TYPE == "bdDbl3_Negate"
    assert BdDbl3Negate.input.long_name == "input"
    assert BdDbl3Negate.ix.short_name == "ix"
    assert BdDbl3Negate.iy.short_name == "iy"
    assert BdDbl3Negate.iz.short_name == "iz"
    assert BdDbl3Negate.output.long_name == "output"
    assert BdDbl3Negate.ox.short_name == "ox"
    assert BdDbl3Negate.oy.short_name == "oy"
    assert BdDbl3Negate.oz.short_name == "oz"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl3_Negate()
    ...
    assert node_fn.typeId.id() == 0x001426A8


def test_negates_each_component(maya_cmds):
    _load_bd_util_nodes(maya_cmds)

    node = maya_cmds.createNode("bdDbl3_Negate")
...
def test_infinity_is_negated_per_component(maya_cmds):
    _load_bd_util_nodes(maya_cmds)
    ...
    output = maya_cmds.getAttr(f"{node}.output")[0]
    assert output == (float("inf"), float("-inf"), -1.0)


def test_signed_zero_is_reversed_per_component(maya_cmds):
    _load_bd_util_nodes(maya_cmds)

    node = maya_cmds.createNode("bdDbl3_Negate")
    maya_cmds.setAttr(f"{node}.input", 0.0, -0.0, 1.0, type="double3")
    output = maya_cmds.getAttr(f"{node}.output")[0]
    assert output[:2] == (0.0, 0.0)
    assert math.copysign(1.0, output[0]) == -1.0
    assert math.copysign(1.0, output[1]) == 1.0
...
def test_connections_existing_accessor_and_scene_round_trip(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    source = nodes.create.bdDbl3_Negate(name="source_neg3")
    target = nodes.create.bdDbl3_Negate(name="target_neg3")
    ...
    existing = nodes.existing.bdDbl3_Negate(target.name)
    assert type(existing) is type(target)

    scene_path = tmp_path / "bd_dbl3_negate.ma"
    ...
    output = reloaded.existing.bdDbl3_Negate("target_neg3").output.get()
