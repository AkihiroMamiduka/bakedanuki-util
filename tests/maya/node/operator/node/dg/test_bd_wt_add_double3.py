...
def test_attributes_defaults_and_type_id(modifier_manager, maya_cmds, maya_om):
    ...
    from bd_util.maya.node.operator.node.dg.bd_wt_add_double3_multi import (
        BdWtAddDouble3Multi,
    )

    assert BdWtAddDouble3Multi.NODE_TYPE == "bdWtAddDouble3Multi"
    assert BdWtAddDouble3Multi.input.long_name == "input"
    assert BdWtAddDouble3Multi.output.long_name == "output"
    assert BdWtAddDouble3Multi.oz.short_name == "oz"

    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdWtAddDouble3Multi()
...
def test_sums_sparse_weighted_vectors_without_normalizing(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdWtAddDouble3Multi(name="weighted")
...
def test_nested_child_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        node = maya_cmds.createNode("bdWtAddDouble3Multi")
        maya_cmds.setAttr(
