...
def test_class_attribute_access(maya_cmds):
    ...
    assert BdDbl3Subtract.NODE_TYPE == "bdDbl3_Subtract"
    assert BdDbl3Subtract.input1.long_name == "input1"
    assert BdDbl3Subtract.i1x.short_name == "i1x"
    assert BdDbl3Subtract.input2.long_name == "input2"
    assert BdDbl3Subtract.i2z.short_name == "i2z"
    assert BdDbl3Subtract.output.long_name == "output"
    assert BdDbl3Subtract.oz.short_name == "oz"

    assert BdDbl3SubtractMulti.NODE_TYPE == "bdDbl3_SubtractMulti"
    assert BdDbl3SubtractMulti.input.long_name == "input"
    assert BdDbl3SubtractMulti.i.short_name == "i"
    assert BdDbl3SubtractMulti.output.long_name == "output"
    assert BdDbl3SubtractMulti.oz.short_name == "oz"
...
def test_defaults_and_fixed_component_wise_subtraction(
    ...
    fixed = nodes.create.bdDbl3_Subtract()
    multi = nodes.create.bdDbl3_SubtractMulti()
    fixed.input1.set((10.0, 20.0, 30.0))
    fixed.input2.set((1.0, 5.0, 7.0))
    modifier_manager.do_it_dg()

    assert fixed.name == "bdDbl3_Subtract1"
    assert multi.name == "bdDbl3_SubtractMulti1"
...
def test_multi_uses_logical_index_order_and_defined_edge_cases(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    node = nodes.create.bdDbl3_SubtractMulti(name="sub")
...
def test_dirty_updates_match_in_all_evaluation_modes(
    ...
    try:
        ...
        fixed = maya_cmds.createNode("bdDbl3_Subtract")
        ...
        multi = maya_cmds.createNode("bdDbl3_SubtractMulti")
...
def test_child_input_dependencies_cover_the_output_compound(
    ...
    for node_type, input_children in (
        ...
        ("bdDbl3_SubtractMulti", ("inputX", "inputY", "inputZ")),
    ):
        node = maya_cmds.createNode(node_type)
...
def test_connection_and_existing_node_accessors(
    ...
    nodes = bdu.Nodes(modifier_manager=modifier_manager)
    fixed = nodes.create.bdDbl3_Subtract(name="fixed")
    multi = nodes.create.bdDbl3_SubtractMulti(name="multi")
    fixed.input1.set((10.0, 20.0, 30.0))
    fixed.input2.set((1.0, 5.0, 7.0))
    ...
    assert isinstance(
        nodes.existing.bdDbl3_Subtract(fixed.name), BdDbl3Subtract
    )
    assert isinstance(
        nodes.existing.bdDbl3_SubtractMulti(multi.name),
        BdDbl3SubtractMulti,
    )
...
def test_both_nodes_survive_scene_save_and_reload(
    ...
    assert fixed_fn.typeId.id() == 0x00142689
    assert multi_fn.typeId.id() == 0x00142688

    scene_path = tmp_path / "bd_dbl3_subtract.ma"
    ...
    assert existing_nodes.existing.bdDbl3_Subtract(
        "fixed"
    ).output.get().as_tuple() == pytest.approx((9.0, 15.0, 23.0))
    assert existing_nodes.existing.bdDbl3_SubtractMulti(
        "multi"
