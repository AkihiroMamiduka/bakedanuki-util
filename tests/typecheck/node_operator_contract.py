...
def transform_rotation_contract(nodes: bdu.Nodes) -> None:
    ...
    assert_type(transform.aim_to_rotate(source), Transform)
    assert_type(
        transform.aim_to_rotate_axis(
            "aim_target",
    ...
    assert_type(joint.aim_to_rotate((10.0, 0.0, 0.0)), Joint)
    assert_type(joint.aim_to_rotate_axis(source), Joint)
    assert_type(
    ...
    assert_type(
        transform.remap_axes_to_rotate_axis(
            x="-y",
            z="x",
            compensate_children=True,
            compensate_child_translate=True,
    ...
    assert_type(joint.remap_axes_to_rotate(x="y", y="-x"), Joint)
    assert_type(joint.remap_axes_to_rotate_axis(y="+z", z="-x"), Joint)
    assert_type(
    ...
    transform.aim_to_rotate(object())  # pyright: ignore[reportArgumentType]
    transform.aim_to_rotate_axis(
        source,
    ...
    transform.remap_axes_to_rotate(
        x="w",  # pyright: ignore[reportArgumentType]
        z="x",
    )
    joint.remap_axes_to_joint_orient(
        x="y",
        z="x",
        joint_child_compensation_attr="axis",  # pyright: ignore[reportArgumentType]
    )
    transform.match_position(
        source,
        compensate_children=1,  # pyright: ignore[reportArgumentType]
    ...
    assert_type(joint.set_rotate((10.0, 20.0, 30.0)), Joint)
    assert_type(joint.set_joint_orient((10.0, 20.0, 30.0)), Joint)
    assert_type(
    ...
    transform.set_rotate("invalid")  # pyright: ignore[reportArgumentType]
    transform.set_joint_orient(  # pyright: ignore[reportAttributeAccessIssue, reportUnknownMemberType]
        (10.0, 20.0, 30.0)
    )
...
def descriptor_contract(compose: ComposeMatrix) -> None:
    ...
    assert_type(matrix_value.translate, bdu.DoubleLinear3)
    assert_type(matrix_value.translate.x, float)
    assert_type(
        matrix_value.translate.as_tuple(),
...
def keyframe_contract(
    ...
    assert_type(
        keyframe.set_keys(
            ((float(frame), float(frame * 2)) for frame in range(3)),
    ...
    keyframe.set_keys(
        [("invalid", 1.0)],  # pyright: ignore[reportArgumentType]
    )
    keyframe.set_keys(
        [(1.0, 2.0, 3.0)],  # pyright: ignore[reportArgumentType]
    )
    keyframe.set_keys(
        [(1.0, 1.0)],
        in_tangent_type="linera",  # pyright: ignore[reportArgumentType]
    ...
    keyframe.set_keys(
        [(1.0, 1.0)],
        frames=[1.0],  # pyright: ignore[reportCallIssue]
    )
    keyframe.get_keys(
...
def invalid_usage_contract(
    nodes: bdu.Nodes,
    c: ComposeMatrix,
    compound: CompoundPlugOperator[Any],
    message: MessagePlugOperator,
    typed: TypedPlugOperator,
