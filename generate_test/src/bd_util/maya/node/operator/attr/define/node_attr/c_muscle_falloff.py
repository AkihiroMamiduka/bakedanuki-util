...
class FalloffDataPlugOperator(
    CompoundPlugOperator["FalloffDataAttrOperator"]
):
    __slots__ = ()
    CHILD_ATTR_NAMES = (
        ("worldMatrixFalloff", "wmf"),
        ("strength", "str"),
        ("radiusStartInner", "radsttinn"),
        ("radiusEndInner", "radendinn"),
        ("radiusStartOuter", "radsttout"),
        ("radiusEndOuter", "radendout"),
        ("length", "len"),
        ("axis", "ax"),
    )

    worldMatrixFalloff = MatrixField()
    wmf = worldMatrixFalloff

    strength = DoubleField(default_value=1.0)
    str = strength

    radiusStartInner = DoubleField(default_value=0.5, min_value=0.0)
    radsttinn = radiusStartInner

    radiusEndInner = DoubleField(default_value=0.5, min_value=0.0)
    radendinn = radiusEndInner

    radiusStartOuter = DoubleField(default_value=1.0, min_value=0.0)
    radsttout = radiusStartOuter

    radiusEndOuter = DoubleField(default_value=1.0, min_value=0.0)
    radendout = radiusEndOuter

    length = DoubleField(default_value=1.0, min_value=0.0)
    len = length
...
class FalloffDataAttrOperator(
    CompoundAttrOperator[FalloffDataPlugOperator]
):
    __slots__ = ()
...
class FalloffDataField(
    CompoundField[FalloffDataAttrOperator, FalloffDataPlugOperator]
):
    __slots__ = ()

    ATTR_CLS = FalloffDataAttrOperator
    PLUG_CLS = FalloffDataPlugOperator
...
class DrawDataPlugOperator(
    ...
    colorInner = Float3Field(default_value=(1.0, 0.0, 0.0), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    colin = colorInner

    colorOuter = Float3Field(default_value=(1.0, 0.699999988079071, 0.10000000149011612), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    colout = colorOuter

    opacity = FloatField(default_value=0.20000000298023224, min_value=0.0, max_value=1.0)
    opa = opacity
