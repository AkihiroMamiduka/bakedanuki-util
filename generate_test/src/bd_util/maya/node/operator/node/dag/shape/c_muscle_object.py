...
from ....attr.define.node_attr.c_muscle_object import (
    BoundingBoxField,
    CenterField,
...
class CMuscleObject(Shape):
    __slots__ = ()

    NODE_TYPE = "cMuscleObject"
    ...
    muscleData = MuscleDataField()
    data = muscleData
    worldMatrixStart = muscleData.worldMatrixStart
    wms = worldMatrixStart
    worldMatrixEnd = muscleData.worldMatrixEnd
    wme = worldMatrixEnd
    worldMatrixStartBase = muscleData.worldMatrixStartBase
    wmsb = worldMatrixStartBase
    worldMatrixEndBase = muscleData.worldMatrixEndBase
    wmeb = worldMatrixEndBase
    meshIn = muscleData.meshIn
    mesh = meshIn
    meshInBase = muscleData.meshInBase
    meshb = meshInBase
    stickyStrength = muscleData.stickyStrength
    stkstr = stickyStrength
    slidingStrength = muscleData.slidingStrength
    sldstr = slidingStrength
    fat = muscleData.fat
    ft = fat
    reverseNormals = muscleData.reverseNormals
    rn = reverseNormals
    type = muscleData.type
    typ = type
    radius = muscleData.radius
    rad = radius
    length = muscleData.length
    len = length
    capsuleAxis = muscleData.capsuleAxis
    cax = capsuleAxis
    relative = muscleData.relative
    rel = relative
    lockStickyWt = muscleData.lockStickyWt
    lkst = lockStickyWt
    lockSlidingWt = muscleData.lockSlidingWt
    lksl = lockSlidingWt
    affectSticky = muscleData.affectSticky
    afstk = affectSticky
    affectSliding = muscleData.affectSliding
    afsld = affectSliding
    userScale = muscleData.userScale
    usc = userScale

    draw = DrawEnumField(default_value=1)
    drw = draw

    shaded = ShadedEnumField(default_value=1)
    shd = shaded

    highlight = FloatField(default_value=1.0, min_value=0.0, max_value=1.0)
    hi = highlight

    highlightShaded = FloatField(default_value=1.0, min_value=0.0, max_value=1.0)
    hish = highlightShaded

    wirecolor = WirecolorField(default_value=(1.0, 0.20000000298023224, 0.20000000298023224), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    wcol = wirecolor
    wirecolorR = wirecolor.wirecolorR
    wcolr = wirecolorR
    wirecolorG = wirecolor.wirecolorG
    wcolg = wirecolorG
    wirecolorB = wirecolor.wirecolorB
    wcolb = wirecolorB

    shadecolor = ShadecolorField(default_value=(0.699999988079071, 0.20000000298023224, 0.25), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    scol = shadecolor
    shadecolorR = shadecolor.shadecolorR
    scolr = shadecolorR
    shadecolorG = shadecolor.shadecolorG
    scolg = shadecolorG
    shadecolorB = shadecolor.shadecolorB
    scolb = shadecolorB

    nSeg = LongField(default_value=3, min_value=1)
    seg = nSeg

    nSides = LongField(default_value=12, min_value=3)
    sid = nSides
