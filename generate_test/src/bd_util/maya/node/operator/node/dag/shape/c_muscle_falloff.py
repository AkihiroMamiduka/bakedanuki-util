...
from ....attr.define.node_attr.c_muscle_falloff import (
    BoundingBoxField,
    CenterField,
...
class CMuscleFalloff(Shape):
    __slots__ = ()

    NODE_TYPE = "cMuscleFalloff"
    ...
    falloffData = FalloffDataField()
    fdata = falloffData
    worldMatrixFalloff = falloffData.worldMatrixFalloff
    wmf = worldMatrixFalloff
    strength = falloffData.strength
    str = strength
    radiusStartInner = falloffData.radiusStartInner
    radsttinn = radiusStartInner
    radiusEndInner = falloffData.radiusEndInner
    radendinn = radiusEndInner
    radiusStartOuter = falloffData.radiusStartOuter
    radsttout = radiusStartOuter
    radiusEndOuter = falloffData.radiusEndOuter
    radendout = radiusEndOuter
    length = falloffData.length
    len = length
    axis = falloffData.axis
    ax = axis
    ...
    hi = highlight
    highlightShaded = drawData.highlightShaded
    hish = highlightShaded
    colorInner = drawData.colorInner
    colin = colorInner
    colorOuter = drawData.colorOuter
    colout = colorOuter
    opacity = drawData.opacity
    opa = opacity
    nSeg = drawData.nSeg
    seg = nSeg
    nSides = drawData.nSides
    sid = nSides
