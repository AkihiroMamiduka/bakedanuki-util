...
class AmbientLight(Shape):
    ...
    ambientShade = FloatField(default_value=0.44999998807907104, min_value=0.0, max_value=1.0)
    as_ = ambientShade

    objectType = CharField(default_value=1, min_value=0, max_value=255, readable=False)
    ot = objectType

    shadowRadius = FloatField(default_value=0.0, min_value=0.0, soft_max_value=1.0)
    sr = shadowRadius

    castSoftShadows = BoolField(default_value=False)
    cw = castSoftShadows

    normalCamera = NormalCameraField(default_value=(1.0, 1.0, 1.0), readable=False)
    n = normalCamera
