...
from ....attr.define.node_attr.c_muscle_spline import (
    BoundingBoxField,
    CenterField,
...
class CMuscleSpline(Shape):
    __slots__ = ()

    NODE_TYPE = "cMuscleSpline"
    ...
    col = color
    TANGENTS = settings.TANGENTS
    TAN = TANGENTS
    tolerance = settings.tolerance
    tol = tolerance
    tangentMode = settings.tangentMode
    tmod = tangentMode
    ...
    LEN = LENGTH
    resetFrame = settings.resetFrame
    rf = resetFrame
    dampenOnSquash = settings.dampenOnSquash
    dmpsq = dampenOnSquash
    dampenOnStretch = settings.dampenOnStretch
    ...
    readData = ReadDataField(multi=True, default_value=(0.0, 0.0), min_value=(0.0, 0.0), max_value=(1.0, 5.0))
    rdat = readData

    squashData = SquashDataField(default_value=(1.0, 0.5, 2.0, 1.0))
    sqdat = squashData
    lenDefault = squashData.lenDefault
    ldef = lenDefault
    lenSquash = squashData.lenSquash
    lsq = lenSquash
    lenStretch = squashData.lenStretch
    lst = lenStretch
    userScale = squashData.userScale
    usc = userScale

    outputData = OutputDataField(multi=True)
    odat = outputData

    outTranslateX = DoubleField()
    otx = outTranslateX

    outTranslateY = DoubleField()
    oty = outTranslateY

    outTranslateZ = DoubleField()
    otz = outTranslateZ

    outRotateX = DoubleField()
    orx = outRotateX

    outRotateY = DoubleField()
    ory = outRotateY

    outRotateZ = DoubleField()
    orz = outRotateZ

    outputSquashData = OutputSquashDataField(default_value=(0.0, 0.0, 0.0))
    osqdat = outputSquashData
    outLen = outputSquashData.outLen
    olen = outLen
    outPctSquash = outputSquashData.outPctSquash
    osq = outPctSquash
    outPctStretch = outputSquashData.outPctStretch
    ost = outPctStretch

    outControlData = OutControlDataField(multi=True)
    ocdata = outControlData

    outPointX = DoubleField()
    opx = outPointX

    outPointY = DoubleField()
    opy = outPointY

    outPointZ = DoubleField()
    opz = outPointZ

    outPointJiggleX = DoubleField()
    opjx = outPointJiggleX

    outPointJiggleY = DoubleField()
    opjy = outPointJiggleY

    outPointJiggleZ = DoubleField()
    opjz = outPointJiggleZ

    outTangentX = DoubleField()
    otgx = outTangentX

    outTangentY = DoubleField()
    otgy = outTangentY

    outTangentZ = DoubleField()
    otgz = outTangentZ
