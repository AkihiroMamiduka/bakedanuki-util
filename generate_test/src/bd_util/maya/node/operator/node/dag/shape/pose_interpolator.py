...
from ....attr.define.node_attr.pose_interpolator import (
    BoundingBoxField,
    CenterField,
...
class PoseInterpolator(Shape):
    ...
    output = FloatField(multi=True, default_value=0.0, writable=False)
    out = output

    regularization = DoubleField(default_value=0.0, min_value=0.0)

    outputSmoothing = FloatField(default_value=0.0, min_value=0.0, max_value=1.0)

    interpolation = InterpolationEnumField(default_value=0)

    allowNegativeWeights = BoolField(default_value=True)

    enableRotation = BoolField(default_value=True)

    enableTranslation = BoolField(default_value=False)

    displayOutput = BoolField(default_value=False)

    displaySpacing = DoubleField(default_value=1.0)

    pose = PoseField(multi=True)

    driver = DriverField(multi=True)

    driverOrientX = DoubleAngleField()

    driverOrientY = DoubleAngleField()

    driverOrientZ = DoubleAngleField()

    driverRotateAxisX = DoubleAngleField()

    driverRotateAxisY = DoubleAngleField()

    driverRotateAxisZ = DoubleAngleField()

    midLayerId = LongField(default_value=-1)
