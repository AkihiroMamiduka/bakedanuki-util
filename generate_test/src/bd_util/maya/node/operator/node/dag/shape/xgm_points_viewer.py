...
from ....attr.define.node_attr.xgm_points_viewer import (
    BoundingBoxField,
    CenterField,
...
class XgmPointsViewer(Shape):
    __slots__ = ()

    NODE_TYPE = "xgmPointsViewer"
    ...
    length = FloatField(default_value=1.0, min_value=0.001)
    len = length

    generate = BoolField(default_value=False)
    gen = generate

    redraw = BoolField(default_value=False)
    rdr = redraw
