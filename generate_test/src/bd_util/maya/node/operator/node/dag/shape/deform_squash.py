...
from ....attr.define.node_attr.deform_squash import (
    BoundingBoxField,
    CenterField,
    DrawOverrideField,
    GhostColorPostField,
...
class DeformSquash(Shape):
    ...
    startSmoothness = DoubleField(default_value=0.0, min_value=0.0, max_value=1.0)
    ss = startSmoothness

    endSmoothness = DoubleField(default_value=0.0, min_value=0.0, max_value=1.0)
    es = endSmoothness

    maxExpandPos = DoubleField(default_value=0.5, min_value=0.01, max_value=0.99)
    mp = maxExpandPos

    expand = DoubleField(default_value=1.0, min_value=0.0, soft_min_value=0.0, soft_max_value=10.0)
    exp = expand

    factor = DoubleField(default_value=0.0, soft_min_value=-10.0, soft_max_value=10.0)
    fac = factor
