...
from ....attr.define.node_attr.xgm_spline_guide import (
    BboxCorner1Field,
    BboxCorner2Field,
    BoundingBoxField,
...
class XgmSplineGuide(Shape):
    __slots__ = ()

    NODE_TYPE = "xgmSplineGuide"
    ...
    uLoc = FloatField(default_value=0.0, min_value=0.0, max_value=1.0)
    ul = uLoc

    vLoc = FloatField(default_value=0.0, min_value=0.0, max_value=1.0)
    vl = vLoc

    faceId = LongField(default_value=0, min_value=0)
    fid = faceId

    bboxCorner1 = BboxCorner1Field(default_value=(0.0, 0.0, 0.0))
    bb1 = bboxCorner1
    ...
    bboxCorner2 = BboxCorner2Field(default_value=(0.0, 0.0, 0.0))
    bb2 = bboxCorner2
    ...
    blend = FloatField(default_value=0.0, min_value=0.0)
    bld = blend

    interp = DataStringField()
    xin = interp

    inputMesh = TypedField()
    is_ = inputMesh

    outputMesh = TypedField(writable=False)
    os = outputMesh

    worldMesh = TypedField(multi=True, writable=False)
    ws = worldMesh

    cachedMesh = TypedField()
    cs = cachedMesh

    toMakeGuide = MessageField()
    tmg = toMakeGuide

    taper = FloatField(default_value=0.20000000298023224)
    tpr = taper

    width = FloatField(default_value=0.10000000149011612, min_value=0.001)
    wid = width

    displayWidth = BoolField(default_value=False)
    dwi = displayWidth

    faceCamera = BoolField(default_value=False)
    fcm = faceCamera
