...
from ....attr.define.node_attr.bif_shape import (
    BboxCorner1Field,
    BboxCorner2Field,
    BoundingBoxField,
...
class AiMotionBlurModeEnumPlugOperator(EnumPlugOperator):
    __slots__ = ()
...
class AiMotionBlurModeEnumAttrOperator(EnumAttrOperator):
    __slots__ = ()
...
class AiMotionBlurModeEnumField(
    EnumField[AiMotionBlurModeEnumAttrOperator, AiMotionBlurModeEnumPlugOperator]
):
    __slots__ = ()
...
class BifShape(Shape):
    ...
    bboxCorner1 = BboxCorner1Field(default_value=(0.0, 0.0, 0.0), writable=False)
    bb1 = bboxCorner1
    ...
    bboxCorner2 = BboxCorner2Field(default_value=(0.0, 0.0, 0.0), writable=False)
    bb2 = bboxCorner2
    ...
    inputSurface = TypedField()
    is_ = inputSurface

    final = TypedField(writable=False)

    proxy = TypedField(writable=False)

    diagnostic = TypedField(writable=False)

    Core__Graph__terminal__final = TypedField()

    Core__Graph__terminal__proxy = TypedField()

    Core__Graph__terminal__diagnostic = TypedField()

    outputSerializedData = DataDoubleArrayField(writable=False)
    od = outputSerializedData
    ...
    aiMotionBlurMode = AiMotionBlurModeEnumField(default_value=0, category="arnold")
