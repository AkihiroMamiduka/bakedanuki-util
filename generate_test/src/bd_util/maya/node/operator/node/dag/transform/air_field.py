...
from ....attr.define.node_attr.air_field import (
    AxialMagnitudeField,
    CurveRadiusField,
    DirectionField,
    FalloffCurveField,
...
class AirField(Transform):
    ...
    direction = DirectionField(default_value=(0.0, 0.0, 0.0), soft_min_value=(-1.0, -1.0, -1.0), soft_max_value=(1.0, 1.0, 1.0))
    d = direction
    ...
    speed = DoubleField(default_value=0.0, min_value=0.0, soft_min_value=0.0, soft_max_value=1.0)
    spd = speed

    inheritVelocity = DoubleField(default_value=0.0, min_value=0.0, soft_max_value=1.0)
    iv = inheritVelocity

    componentOnly = BoolField(default_value=False)
    co = componentOnly

    spread = DoubleField(default_value=0.0, min_value=0.0, max_value=1.0)
    spr = spread

    enableSpread = BoolField(default_value=False)
    es = enableSpread

    inheritRotation = BoolField(default_value=False)
    ir = inheritRotation
