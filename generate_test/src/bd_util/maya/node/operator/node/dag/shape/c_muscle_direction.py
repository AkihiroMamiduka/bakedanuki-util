...
from ....attr.define.node_attr.c_muscle_direction import (
    BoundingBoxField,
    CenterField,
...
class CMuscleDirection(Shape):
    __slots__ = ()

    NODE_TYPE = "cMuscleDirection"
    ...
    dirData = DirDataField()
    ddata = dirData
    worldMatrixA = dirData.worldMatrixA
    wma = worldMatrixA
    strength = dirData.strength
    str = strength
    falloffInner = dirData.falloffInner
    fi = falloffInner
    falloffOuter = dirData.falloffOuter
    fo = falloffOuter
    type = dirData.type
    typ = type
    length = dirData.length
    len = length
    dirAxis = dirData.dirAxis
    dax = dirAxis
    lockDirWt = dirData.lockDirWt
    lkdi = lockDirWt
    ...
    inColor = InColorField(default_value=(1.0, 0.0, 0.0), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    icol = inColor
    ...
    outColor = OutColorField(default_value=(1.0, 0.8799999952316284, 0.0), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    ocol = outColor
