...
from ....attr.define.node_attr.hair_constraint import (
    BoundingBoxField,
    CenterField,
    DrawOverrideField,
    GhostColorPostField,
...
class PointMethodEnumPlugOperator(EnumPlugOperator):
    __slots__ = ()
...
class PointMethodEnumAttrOperator(EnumAttrOperator):
    __slots__ = ()
...
class HairConstraint(Shape):
    ...
    stiffness = DoubleField(default_value=0.5, soft_min_value=0.0, soft_max_value=1.0)
    stf = stiffness
    ...
    uDistance = DoubleField(default_value=1.0, min_value=0.0, soft_max_value=50.0)
    udi = uDistance
