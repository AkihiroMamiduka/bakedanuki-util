...
from ....attr.define.node_attr.volume_light import (
    BoundingBoxField,
    CenterField,
    ColorField,
    ColorRangeField,
...
class LightShapeEnumPlugOperator(EnumPlugOperator):
    __slots__ = ()
...
class LightShapeEnumAttrOperator(EnumAttrOperator):
    __slots__ = ()
...
class VolumeLightDirEnumPlugOperator(EnumPlugOperator):
    __slots__ = ()
...
class VolumeLightDirEnumAttrOperator(EnumAttrOperator):
    __slots__ = ()
...
class VolumeLight(Shape):
    ...
    coneEndRadius = FloatField(default_value=0.0, soft_min_value=0.0, soft_max_value=2.0)
    cer = coneEndRadius

    colorRange = ColorRangeField(multi=True)
    crg = colorRange

    colorRange_ColorR = FloatField()
    crgcr = colorRange_ColorR

    colorRange_ColorG = FloatField()
    crgcg = colorRange_ColorG

    colorRange_ColorB = FloatField()
    crgcb = colorRange_ColorB

    penumbra = PenumbraField(multi=True, default_value=(0.0, 0.0, 0.0))
    pen = penumbra

    emitAmbient = BoolField(default_value=False)
    ema = emitAmbient

    lightAngle = FloatField(default_value=0.0, soft_min_value=0.0, soft_max_value=360.0)
    lang = lightAngle
