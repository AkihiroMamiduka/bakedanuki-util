...
from ....attr.define.node_attr.c_muscle_smart_collide import (
    BoundingBoxField,
    CenterField,
    CollideDataField,
...
class CMuscleSmartCollide(Shape):
    __slots__ = ()

    NODE_TYPE = "cMuscleSmartCollide"
    ...
    collideData = CollideDataField()
    cdata = collideData
    worldMatrixA = collideData.worldMatrixA
    wma = worldMatrixA
    worldMatrixB = collideData.worldMatrixB
    wmb = worldMatrixB
    worldMatrixABase = collideData.worldMatrixABase
    wmab = worldMatrixABase
    worldMatrixBBase = collideData.worldMatrixBBase
    wmbb = worldMatrixBBase
    worldMatrixPlane = collideData.worldMatrixPlane
    wmpln = worldMatrixPlane
    worldMatrixPlaneBase = collideData.worldMatrixPlaneBase
    wmplnbase = worldMatrixPlaneBase
    enable = collideData.enable
    ena = enable
    collideMode = collideData.collideMode
    colmod = collideMode
    axis = collideData.axis
    ax = axis
    triggerMin = collideData.triggerMin
    trgmin = triggerMin
    angleMin = collideData.angleMin
    angmin = angleMin
    angleMax = collideData.angleMax
    angmax = angleMax
    bias = collideData.bias
    bis = bias
    biasAdjust = collideData.biasAdjust
    bisadj = biasAdjust
    userScale = collideData.userScale
    usrscl = userScale
    manualScale = collideData.manualScale
    manscl = manualScale
    SMOOTH_PRE = collideData.SMOOTH_PRE
    SMTHPRE = SMOOTH_PRE
    smoothIterationsPre = collideData.smoothIterationsPre
    smipre = smoothIterationsPre
    smoothStrengthPre = collideData.smoothStrengthPre
    smstrpre = smoothStrengthPre
    smoothHoldPre = collideData.smoothHoldPre
    hldpre = smoothHoldPre
    MOVEMENT = collideData.MOVEMENT
    MOVE = MOVEMENT
    bulkA = collideData.bulkA
    blka = bulkA
    bulkB = collideData.bulkB
    blkb = bulkB
    bulkAngularA = collideData.bulkAngularA
    blkanga = bulkAngularA
    bulkAngularB = collideData.bulkAngularB
    blkangb = bulkAngularB
    bulkWidenA = collideData.bulkWidenA
    blkwida = bulkWidenA
    bulkWidenB = collideData.bulkWidenB
    blkwidb = bulkWidenB
    slideA = collideData.slideA
    slda = slideA
    slideB = collideData.slideB
    sldb = slideB
    slideRearA = collideData.slideRearA
    sldrera = slideRearA
    slideRearB = collideData.slideRearB
    sldrerb = slideRearB
    slideAngularA = collideData.slideAngularA
    sldanga = slideAngularA
    slideAngularB = collideData.slideAngularB
    sldangb = slideAngularB
    slideAngularRearA = collideData.slideAngularRearA
    sldangrera = slideAngularRearA
    slideAngularRearB = collideData.slideAngularRearB
    sldangrerb = slideAngularRearB
    wrinkleA = collideData.wrinkleA
    wrka = wrinkleA
    wrinkleB = collideData.wrinkleB
    wrkb = wrinkleB
    wrinkleSpread = collideData.wrinkleSpread
    wrkspr = wrinkleSpread
    COLLISION = collideData.COLLISION
    COLL = COLLISION
    flattenA = collideData.flattenA
    flta = flattenA
    flattenB = collideData.flattenB
    fltb = flattenB
    rigidA = collideData.rigidA
    riga = rigidA
    rigidB = collideData.rigidB
    rigb = rigidB
    collisionBlurIterations = collideData.collisionBlurIterations
    colblrit = collisionBlurIterations
    volumizeA = collideData.volumizeA
    vlma = volumizeA
    volumizeB = collideData.volumizeB
    vlmb = volumizeB
    volumizeOffset = collideData.volumizeOffset
    vlmoff = volumizeOffset
    volumizePuff = collideData.volumizePuff
    vlmpuf = volumizePuff
    volumizeDist = collideData.volumizeDist
    vlmd = volumizeDist
    volumizeFalloff = collideData.volumizeFalloff
    vlmfall = volumizeFalloff
    SMOOTH_POST = collideData.SMOOTH_POST
    SMTHPST = SMOOTH_POST
    smoothIterationsPost = collideData.smoothIterationsPost
    smipst = smoothIterationsPost
    smoothStrengthPost = collideData.smoothStrengthPost
    smstrpst = smoothStrengthPost
    smoothHoldPost = collideData.smoothHoldPost
    hldpst = smoothHoldPost
    lockSmartWt = collideData.lockSmartWt
    lksmrt = lockSmartWt

    drawData = DrawDataField()
    ddata = drawData
    DISPLAY = drawData.DISPLAY
    DSPL = DISPLAY
    draw = drawData.draw
    drw = draw
    shaded = drawData.shaded
    shd = shaded
    highlight = drawData.highlight
    hi = highlight
    color = drawData.color
    col = color
    colorTrigger = drawData.colorTrigger
    coltrg = colorTrigger
