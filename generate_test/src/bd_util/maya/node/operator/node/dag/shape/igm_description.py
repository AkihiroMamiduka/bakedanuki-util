...
from ....attr.define.node_attr.igm_description import (
    BaseColorField,
    BoundingBoxField,
    CenterField,
...
class IgmDescription(Shape):
    __slots__ = ()

    NODE_TYPE = "igmDescription"
    ...
    geom = DataStringField()
    geo = geom

    faceIds = LongField(multi=True, default_value=-1)
    fid = faceIds

    density = FloatField(default_value=0.10000000149011612, min_value=0.0)
    den = density

    interpStyle = LongField(default_value=0, min_value=0, max_value=2)
    its = interpStyle

    mask = DataStringField()
    msk = mask

    xuvDir = DataStringField()
    xuv = xuvDir

    baseColor = BaseColorField(default_value=(0.46700000762939453, 0.31299999356269836, 0.3100000023841858))
    bcl = baseColor
    ...
    tipColor = TipColorField(default_value=(0.9210000038146973, 0.8330000042915344, 0.017000000923871994))
    tcl = tipColor
    ...
    displayType = LongField(default_value=0, min_value=0, max_value=1)
    dt = displayType

    path = DataStringField()
    pth = path

    tickle = BoolField(default_value=False)
    tkl = tickle

    length = FloatField(default_value=0.5, min_value=0.0)
    len = length

    width = FloatField(default_value=0.10000000149011612, min_value=0.0)
    wid = width

    cachedData = TypedField()
    cdt = cachedData

    loadCache = BoolField(default_value=False)
    loc = loadCache

    forceLoadCache = LongField(default_value=0)
    floc = forceLoadCache
