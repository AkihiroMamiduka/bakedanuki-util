...
from ....attr.define.node_attr.mash_points import (
    BoundingBoxField,
    CenterField,
    ColourField,
...
class ChannelEnumPlugOperator(EnumPlugOperator):
    __slots__ = ()
...
class ChannelEnumAttrOperator(EnumAttrOperator):
    __slots__ = ()
...
class ChannelEnumField(
    EnumField[ChannelEnumAttrOperator, ChannelEnumPlugOperator]
):
    __slots__ = ()

    ATTR_CLS = ChannelEnumAttrOperator
    PLUG_CLS = ChannelEnumPlugOperator


class MASH_Points(Shape):
    __slots__ = ()

    NODE_TYPE = "MASH_Points"
    ...
    dPointSize = FloatField(default_value=3.0, min_value=1.0, soft_max_value=5.0)
    ...
    channel = ChannelEnumField(default_value=1)

    nextState = TypedField(multi=True)
    nst = nextState

    startState = TypedField(multi=True)
    sst = startState

    currentState = TypedField(multi=True)
    cst = currentState

    useCustomChannel = BoolField(default_value=False)

    customChannel = DataStringField()
