...
from ....attr.define.node_attr.height_field import (
    BoundingBoxField,
...
class HeightField(Shape):
    ...
    resolution = LongField(default_value=10, min_value=1, max_value=2000, soft_max_value=100)
    res = resolution
    ...
    displacement = FloatField(default_value=0.0, soft_min_value=0.0, soft_max_value=1.0)
    dis = displacement

    heightScale = FloatField(default_value=1.0, soft_min_value=0.0, soft_max_value=1.0)
    hc = heightScale

    cacheNeedsRebuilding = BoolField(default_value=True, writable=False)
    cnr = cacheNeedsRebuilding
