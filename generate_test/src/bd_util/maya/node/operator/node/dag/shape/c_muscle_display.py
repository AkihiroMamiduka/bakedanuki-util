...
from ....attr.define.node_attr.c_muscle_display import (
    BoundingBoxField,
    CenterField,
    ColorAField,
    ColorBField,
...
class DrawEnumAttrOperator(EnumAttrOperator):
    ...
    NAME_MAP = {
        ...
        SLIDING: "sliding",
        DIRECTION: "direction",
        SMARTREGIONA: "smartregiona",
        SMARTREGIONB: "smartregionb",
        SMARTBULK: "smartbulk",
        SMARTBULKANGULAR: "smartbulkangular",
        SMARTBULKWIDEN: "smartbulkwiden",
        SMARTSLIDE: "smartslide",
        SMARTSLIDEANGULAR: "smartslideangular",
        SMARTSMOOTH: "smartsmooth",
        SMARTWRINKLE: "smartwrinkle",
        SMARTFLATTEN: "smartflatten",
        SMARTVOLUMIZE: "smartvolumize",
        FORCE: "force",
        JIGGLE: "jiggle",
        CYCLE: "cycle",
        REST: "rest",
        ...
        WRINKLE: "wrinkle",
        SMOOTH: "smooth",
        SMOOTHCOMPRESS: "smoothcompress",
        SMOOTHEXPAND: "smoothexpand",
        SELFCOLLISION: "selfcollision",
        SELFRIGIDITY: "selfrigidity",
        SELFVOLUMIZE: "selfVolumize",
    }
...
class CMuscleDisplay(Shape):
    __slots__ = ()

    NODE_TYPE = "cMuscleDisplay"
    ...
    draw = DrawEnumField(default_value=0)
    drw = draw

    colorA = ColorAField(default_value=(1.0, 0.0, 0.0), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    colA = colorA
    ...
    colorB = ColorBField(default_value=(1.0, 1.0, 0.0), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    colB = colorB
    ...
    colorC = ColorCField(default_value=(0.0, 0.30000001192092896, 0.5), min_value=(0.0, 0.0, 0.0), max_value=(1.0, 1.0, 1.0))
    colC = colorC
    colorCR = colorC.colorCR
    colCr = colorCR
    colorCG = colorC.colorCG
    colCg = colorCG
    colorCB = colorC.colorCB
    colCb = colorCB

    msgSystem = MessageField()
    msgs = msgSystem

    index = LongField(default_value=0)
    idx = index

    ptSize = LongField(default_value=8, min_value=1, max_value=64)
    psz = ptSize
    ...
    ptPos = PtPosField(multi=True, default_value=(0.0, 0.0, 0.0))
    pos = ptPos

    numPts = LongField(default_value=0)
    num = numPts

    numCols = LongField(default_value=0)
    ncl = numCols
    ...
    weightsForce = DoubleField(multi=True, default_value=0.0)
    wtfrc = weightsForce

    jiggle = DoubleField(multi=True, default_value=1.0, min_value=0.0, max_value=1.0)
    jig = jiggle

    cycle = DoubleField(multi=True, default_value=1.0, min_value=0.0, max_value=1.0)
    cyc = cycle

    rest = DoubleField(multi=True, default_value=1.0, min_value=0.0, max_value=1.0)
    rst = rest

    weightsRelax = DoubleField(multi=True, default_value=0.0)
    wtrlx = weightsRelax

    weightsWrinkle = DoubleField(multi=True, default_value=0.0)
    wtwrk = weightsWrinkle

    weightsDetail = DoubleField(multi=True, default_value=1.0)
    wtdtl = weightsDetail

    weightsSmooth = DoubleField(multi=True, default_value=0.0)
    wtsmt = weightsSmooth
