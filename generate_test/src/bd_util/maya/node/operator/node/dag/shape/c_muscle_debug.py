...
from ....attr.define.node_attr.c_muscle_debug import (
    BoundingBoxField,
    CenterField,
...
class CMuscleDebug(Shape):
    __slots__ = ()

    NODE_TYPE = "cMuscleDebug"
    ...
    edgePath = EdgePathField(multi=True)
    epath = edgePath

    edgeColorR = DoubleField()
    ecr = edgeColorR

    edgeColorG = DoubleField()
    ecg = edgeColorG

    edgeColorB = DoubleField()
    ecb = edgeColorB

    pointGroup = PointGroupField(multi=True)
    pgrp = pointGroup

    pointColorR = DoubleField()
    pcr = pointColorR

    pointColorG = DoubleField()
    pcg = pointColorG

    pointColorB = DoubleField()
    pcb = pointColorB

    lineWidth = FloatField(default_value=5.0, min_value=1.0)
    lwid = lineWidth

    pointSize = FloatField(default_value=12.0, min_value=1.0, max_value=64.0)
    psiz = pointSize
