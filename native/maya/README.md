...
版それぞれの乗算・加算・減算・安全除算・最小値・最大値・Clamp・Map Range・Absolute・Negate・Conditionについて、固定入力ノード、
可変長入力ノード、空入力、sparse 配列、logical index 順、除数epsilon、`NaN`、無限値、
符号付きゼロ、逆転した上下限、方向付き範囲、Source幅0、外挿、絶対値、符号反転、条件分岐、最初の一致、element 削除、compound child の直接要求、compound dirty、DG / Serial /
