...
```python
node = cmds.createNode("bdMultDouble3Pair")
...
```python
node = cmds.createNode("bdMultDouble3Multi")
...
`bdMultDoublePair` のチェーンと `bdMultDoubleMulti` の再実行可能なベンチマークは次で
実行します。
...
[bdDouble Multiplication Benchmark](bd-mult-double-benchmark.md) に記録します。
